...
def demo_recuperar_pendentes():
    ...
    if not base or not demos:
        return

    for demo_id, sessao in demos.items():
        if not isinstance(sessao, dict) or not sessao.get("active"):
            continue
        last_id = str(sessao.get("last_round_id", ""))
        start = len(base)
        if last_id:
        ...
        for i in range(start, len(base)):
            atual = _demo_processar_sessao(
                atual, base[i], cores(base[:i])
            )
            if not atual.get("active"):
                break

        with LOCK:
            ESTADO.setdefault("demo_contas", {})[str(demo_id)] = atual
            salvar_json(BANCO, ESTADO)
        if postgres_configurado():
            postgres_demo_salvar(demo_id, atual)
...
def _optimizer_colors():
    data = historico_validado(30000)
    rows = list(data.get("rodadas", []) or [])
    colors = []
    ids = []
    for r in rows:
...
def _optimizer_cache_accepts(item, min_rate_enabled, min_rate, dd_enabled, dd_limit):
    if min_rate_enabled and float(item.get("rate", 0.0)) + 0.0001 < float(min_rate):
        return False
...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        # 0=W, 1=R, 2=B. NumPy mantém a base compacta em memória.
        colors = np.fromiter(
        ...
        max_count_req = max(1, min(1000, int(payload.get("max_count", 1000))))
        ...
        space_key = _optimizer_space_key(hist_hash, cfg, contrary)
        cache_payload = {
            "engine": "v56.4-pg-exact-cache",
            "history": hist_hash, "gales": gales, "modes": modes,
            "max_count": max_count_req, "consensus": consensuses,
            "contrarian": contrary, "cfg": cfg,
            "min_rate_enabled": min_rate_enabled, "min_rate": min_rate,
            "dd_enabled": dd_enabled, "dd_limit": dd_limit
        }
        ...
        if isinstance(cached, dict):
            ...
            _optimizer_job_update(
                ...
                phase="CACHE V56.2 PRONTO", result=cached
            )
            return

        total_configs = len(gales) * len(modes) * max_count_req * len(consensuses)
        _optimizer_job_update(
            job_id, status="running", phase="PREPARANDO MOTOR + CACHE V56.4",
            tested=0, total=total_configs, progress=0.0
        )

        _optimizer_numba_core(
        ...
        _optimizer_job_update(
            job_id, status="running", phase="MOTOR V56.4 PRONTO",
            tested=0, total=total_configs, progress=0.0
        )

        # Padrões calculados uma única vez para todo o job.
        ...
        for gale in gales:
            for mode in modes:
                cached_pairs = _optimizer_pg_cached_pairs(
                    space_key, gale, mode, max_count_req, consensuses
                )
                expected_pair_count = max_count_req * len(consensuses)
                _optimizer_job_update(
                    job_id,
                    phase=("CACHE V56.4: %d/%d PRONTAS" % (
                        len(cached_pairs), expected_pair_count
                    )),
                    gale=gale, gale_mode=mode,
                ...
                _optimizer_job_update(
                    job_id, phase="RANKING ESPARSO 2044 + CACHE",
                    gale=gale, gale_mode=mode,
                    tested=tested, total=total_configs
                )

                ranked = []
                # Ranking EXATO das 2044 individuais, mas sem máscara de 30k
                # e sem .find() byte-a-byte em cada simulação.
                for idx, cand0 in enumerate(base):
                    positions = cand0["positions"]
                    if not positions:
                        continue
                    pos_np = np.asarray(positions, dtype=np.int32)

                    for entry, target_code in (("R", 1), ("B", 2)):
                        targets = np.zeros(n, dtype=np.uint8)
                        targets[pos_np] = target_code
                        r = _optimizer_simulate_numba(
                            colors, positions, targets, cfg, gale, mode
                        )
                        if int(r.get("entries", 0)) >= 5:
                            ranked.append({
                                "plen": cand0["plen"], "mask": cand0["mask"],
                                "pattern": cand0["pattern"], "entry": entry,
                                "positions": positions,
                    ...
                    if idx % 64 == 0:
                        _optimizer_job_update(
                            job_id, phase="RANKING ESPARSO 2044",
                            phase_current=min(2044, idx * 2),
                            phase_total=2044
                        )

                ranked.sort(key=lambda x: (x["profit"], x["rate"]), reverse=True)
                ...
                for count in range(1, max_count + 1):
                    cand = ranked[count - 1]
                    pos_np = np.asarray(cand["positions"], dtype=np.int32)
                    if cand["entry"] == "R":
                        votes_r[pos_np] += 1
                    else:
                        votes_b[pos_np] += 1

                    active = votes_r + votes_b
                    winner = np.maximum(votes_r, votes_b)
                    natural = np.where(votes_r > votes_b, 1, np.where(votes_b > votes_r, 2, 0)).astype(np.uint8)
                    tied_or_zero = (active == 0) | (votes_r == votes_b)

                    selected = ranked[:count]

                    for con in consensuses:
                        cached_item = cached_pairs.get((count, con))
                        if cached_item is not None:
                            tested += 1
                            rate = float(cached_item.get("rate", 0.0))
                            dd = float(cached_item.get("max_dd", 0.0))
                            profit = float(cached_item.get("profit", float("-inf")))
                            passes = _optimizer_cache_accepts(
                                cached_item,
                                min_rate_enabled, min_rate,
                                dd_enabled, dd_limit
                            )
                            if passes and (
                                profit > best_profit or
                                (abs(profit - best_profit) < 1e-9 and dd < best_dd)
                            ):
                                best_profit = profit
                                best_dd = dd
                                best_result = dict(cached_item.get("result", {}) or {})
                                best_strategy = _optimizer_strategy_dict(
                                    selected, gale, mode, con, cfg, contrary
                                )
                            if tested % 250 == 0:
                                _optimizer_job_update(
                                    job_id,
                                    phase="LENDO CACHE EXATO V56.4",
                                    tested=tested, total=total_configs,
                                    progress=min(0.999, tested / float(max(1, total_configs))),
                                    best_profit=(best_profit if best_profit != float("-inf") else 0.0),
                                    count=count, consensus=con, gale=gale, gale_mode=mode
                                )
                            continue

                        required = 50.0001 if con <= 50 else float(con)
                        # Tudo abaixo é vetorizado em C/NumPy.
                        enough = (~tied_or_zero) & ((winner.astype(np.float32) * 100.0) >= (active.astype(np.float32) * required))
                        targets = np.where(enough, natural, 0).astype(np.uint8)
                        if contrary:
                            targets = np.where(targets == 1, 2, np.where(targets == 2, 1, 0)).astype(np.uint8)

                        signal_positions = np.flatnonzero(targets).astype(np.int32)
                        r = _optimizer_simulate_numba(
                            colors, signal_positions, targets, cfg, gale, mode
                        )
                        ...
                        profit = float(r.get("profit", float("-inf")))
                        passes = (not min_rate_enabled or rate + 0.0001 >= min_rate)
                        if dd_enabled and dd > dd_limit:
                            passes = False
                        ...
                        if len(db_batch) >= 250:
                            _optimizer_pg_insert_batch(
                                space_key, gale, mode, db_batch
                            )
                            db_batch = []
                        ...
                        if tested % 250 == 0:
                            _optimizer_job_update(
                                job_id, phase="VARREDURA EXATA + CACHE V56.4",
                                tested=tested, total=total_configs,
                                progress=min(0.999, tested / float(max(1, total_configs))),
                                best_profit=(best_profit if best_profit != float("-inf") else 0.0),
                                count=count, consensus=con, gale=gale, gale_mode=mode
        ...
        if best_strategy is None:
            result_payload = {
                ...
                "tested": tested, "total": total_configs,
                "elapsed": time.time() - started, "history_count": n,
                "engine": "V56.4 CACHE POSTGRES EXACT", "persistent_config_cache": True
            }
        else:
            result_payload = {
                "ok": True, "strategy": best_strategy, "result": best_result,
                "tested": tested, "total": total_configs,
                "elapsed": time.time() - started, "history_count": n,
                "history_hash": hist_hash, "cached": False,
                "engine": "V56.4 CACHE POSTGRES EXACT", "persistent_config_cache": True
            }
            with OPTIMIZER_CACHE_LOCK:
        ...
        _optimizer_job_update(
            job_id, status="done", phase="CONCLUÍDO V56.4",
            progress=1.0, tested=tested, total=total_configs,
            result=result_payload
        )

    except Exception as exc:
        _optimizer_job_update(
            job_id, status="error", phase="ERRO",
            error=str(exc), progress=1.0
        )
        print("Optimizer remoto V56.4 erro:", repr(exc), flush=True)
...
def main():
    print("V56.4 CACHE POSTGRES EXATO + NUMBA: janela 30000 + configurações persistentes: iniciando servidor...", flush=True)
    carregar_estado()
