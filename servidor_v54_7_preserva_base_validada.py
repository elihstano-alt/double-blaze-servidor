...
def worker_bootstrap_1500():
    # Aguarda WebSocket e HTTP subirem para poder encaixar a cauda pública no LIVE.
    time.sleep(25)
    try:
        with LOCK:
            estado_boot = dict(ESTADO.get("bootstrap_1500", {}) or {})
            base_atual = ordenar_rodadas_canonicas(list(ESTADO.get("rodadas", [])))

        # Primeiro protege a base restaurada do PostgreSQL. Isso evita que cada
        # novo deploy volte de, por exemplo, 1581 para exatamente 1500.
        diag_atual = diagnosticar_continuidade_historico(base_atual, limite_gap=45)
        if (
            len(base_atual) >= 1500
            and int(diag_atual.get("lacunas", 0)) == 0
            and int(diag_atual.get("total_validado", 0)) == len(base_atual)
        ):
            with LOCK:
                ESTADO["bootstrap_1500"] = {
                    "status": "preservado",
                    ...
                    "meta": 1500,
                    "motivo": "postgres_continuo_preservado"
                }
                salvar_json(BANCO, ESTADO)
            print("Bootstrap 1500: base contínua existente preservada:", len(base_atual), flush=True)
            return

        # Compatibilidade com estado local persistente quando existir.
        if str(estado_boot.get("status", "")) in ("pronto", "preservado") and int(estado_boot.get("quantidade", 0)) >= 1500:
            return
        resultado = bootstrap_1500_mais_recentes(1500, 3)
        print("Bootstrap 1500:", resultado, flush=True)
    except Exception as exc:
        with LOCK:
            ESTADO["bootstrap_1500"] = {
                "status": "erro",
...
def main():
    print("V54.7 Preserva base validada + Auditoria contínua + janela 5000: iniciando servidor...", flush=True)
    carregar_estado()
