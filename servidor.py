...
def _optimizer_excluded_pairs(blocked_signatures, gale, mode, contrary):
    """
    V58.20: converte assinaturas das estratégias SALVAS no app em pares
    (quantidade_de_estrategias, consenso) que devem ser pulados nesta combinação
    de Gale/modo/contrária.

    consenso = -1 significa: pular qualquer consenso para estratégia single,
    pois a assinatura final de 1 estratégia não carrega consenso.
    """
    counts = []
    consensuses = []
    ...
    for raw in blocked_signatures or []:
        ...
        # single|PADRAO|ENTRY|g0|m0|invfalse
        m_single = re.match(
            r"^single\|.*\|g(\d+)\|m(\d+)\|inv(true|false)(?:\|wp(?:true|false)\|w[0-9.]+)?$",
            sig,
            flags=re.IGNORECASE
        )
        if m_single:
            sg = int(m_single.group(1))
            sm = int(m_single.group(2))
            sinv = m_single.group(3).lower() == "true"
            if sg == int(gale) and sm == int(mode) and sinv == inv_target:
                counts.append(1)
                consensuses.append(-1)
            continue

        # ensemble|<single;single;...>|g0|m0|c70|invfalse
        m_ens = re.match(
            r"^ensemble\|(.*)\|g(\d+)\|m(\d+)\|c(\d+)\|inv(true|false)(?:\|wp(?:true|false)\|w[0-9.]+)?$",
            sig,
            flags=re.IGNORECASE
        )
        if m_ens:
            body = str(m_ens.group(1))
            sg = int(m_ens.group(2))
            sm = int(m_ens.group(3))
            scon = int(m_ens.group(4))
            sinv = m_ens.group(5).lower() == "true"
            if sg == int(gale) and sm == int(mode) and sinv == inv_target:
                count = 0 if body == "" else body.count(";") + 1
                if count > 0:
...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        cache_payload = {
            "engine": "v58.22-white-protection-exact",
            "history": hist_hash,
        ...
        for gale in gales:
            for mode in modes:
                ...
                skip_counts_np, skip_cons_np = _optimizer_excluded_pairs(
                    blocked_signatures, gale, mode, contrary
                )
