...
def carregar_estado():
    """
    V58.15 — restauração não destrutiva.
    PostgreSQL é a única persistência que sobrevive a deploy no Railway.
    Nunca substitui uma base existente por vazio.
    """
    global ESTADO
    ...
    if postgres_configurado():
        ...
        if rodadas_pg:
            with LOCK:
                ESTADO["rodadas"] = ordenar_rodadas_canonicas(rodadas_pg)
                ESTADO["ultima_atualizacao"] = agora_brasilia()
    ...
    # Sem DATABASE_URL, o arquivo local NÃO sobrevive a um novo container.
    ESTADO["persistencia_configurada"] = bool(postgres_configurado())
    ESTADO["persistencia_modo"] = (
        "postgresql" if postgres_configurado()
        else "local_ephemero_nao_sobrevive_deploy"
    )
...
def _demo_sinal_estrategia(strategy, previous_colors):
    ...
    if strategy_type == "meta_adaptive":
        current = strategy.get("current_strategy", {})
        if isinstance(current, dict):
            return _demo_sinal_estrategia(current, previous_colors)
        return ""
...
def _demo_operacao(session, rodada, kind, target, stake, gale, reason=""):
    ...
    ops.insert(0, {
        ...
        # V58.24: auditoria financeira da proteção do branco.
        "protecao_branco_ativa": bool(session.get("last_white_protection_enabled", False)),
        "protecao_branco_valor": round(float(session.get("last_white_protection_stake", 0.0)), 2),
        "protecao_branco_retorno": round(float(session.get("last_white_protection_return", 0.0)), 2),
        "resultado_liquido_operacao": round(float(session.get("last_operation_net", 0.0)), 2)
    })
    session["operations"] = ops[:500]



def _demo_processar_espelho(session, rodada, previous_colors):
    if not bool(session.get("mirror_enabled", False)):
        return session

    rid = str((rodada or {}).get("id", ""))
    rdt = str((rodada or {}).get("data_hora", ""))
    key = rid or rdt
    if key and key == str(session.get("mirror_last_round_key", "")):
        return session

    activation = str(session.get("mirror_activation_round_data_hora", "")).strip()
    activation_ep = _epoch_brasilia(activation) if activation else 0.0
    rodada_ep = momento_efetivo_epoch(rodada)
    if activation_ep > 0.0 and rodada_ep > 0.0 and rodada_ep <= activation_ep + 0.001:
        return session

    strategy = dict(session.get("strategy", {}) or {})
    signal = _demo_sinal_estrategia(strategy, previous_colors)
    if signal not in ("R", "B"):
        session["mirror_last_round_key"] = key
        return session

    target_a = signal
    target_b = "B" if signal == "R" else "R"
    actual = str((rodada or {}).get("cor", ""))
    stake = max(0.01, float(session.get("entry_value", strategy.get("stake", 10.0))))

    bank_a = float(session.get("mirror_bank_a", session.get("bank", 0.0)))
    bank_b = float(session.get("mirror_bank_b", session.get("bank", 0.0)))

    # V58.23 — a proteção no branco pertence a CADA lado do espelho.
    # A e B fazem sua própria proteção simultaneamente à entrada R/P.
    protection_enabled = bool(session.get("white_protection_enabled", False))
    protection_stake = (
        max(0.0, float(session.get("white_protection_stake", 1.0)))
        if protection_enabled else 0.0
    )
    protection_payout = max(
        1.0, float(session.get("white_protection_payout", 14.0))
    )
    total_stake = stake + protection_stake

    # Cada banca é independente. Não executar artificialmente se uma das
    # duas já não tiver saldo suficiente para principal + proteção.
    can_a = bank_a >= total_stake
    can_b = bank_b >= total_stake

    win_a = actual == target_a
    win_b = actual == target_b

    if can_a:
        bank_a -= total_stake
        if protection_enabled and actual == "W":
            bank_a += protection_stake * protection_payout
        if win_a:
            bank_a += stake * 2.0
    ...
    if can_b:
        bank_b -= total_stake
        if protection_enabled and actual == "W":
            bank_b += protection_stake * protection_payout
        if win_b:
            bank_b += stake * 2.0

    session["mirror_bank_a"] = bank_a
    session["mirror_bank_b"] = bank_b
    session["mirror_entries"] = int(session.get("mirror_entries", 0)) + 1
    # V58.27 — contagem W/L do espelho:
    # - acertou a cor principal de A/B => WIN daquele lado;
    # - saiu BRANCO com proteção ativa => WIN para A e WIN para B na CONTAGEM;
    # - o crédito financeiro continua sendo calculado uma única vez em cada banca,
    #   acima, sem qualquer duplicação artificial.
    white_protection_win = bool(protection_enabled and actual == "W")

    # V58.29: separa os tipos de resultado sem alterar a banca.
    if can_a:
        if white_protection_win:
            session["mirror_white_wins_a"] = int(session.get("mirror_white_wins_a", 0)) + 1
        elif win_a:
            session["mirror_color_wins_a"] = int(session.get("mirror_color_wins_a", 0)) + 1
        else:
            session["mirror_real_losses_a"] = int(session.get("mirror_real_losses_a", 0)) + 1
    if can_b:
        if white_protection_win:
            session["mirror_white_wins_b"] = int(session.get("mirror_white_wins_b", 0)) + 1
        elif win_b:
            session["mirror_color_wins_b"] = int(session.get("mirror_color_wins_b", 0)) + 1
        else:
            session["mirror_real_losses_b"] = int(session.get("mirror_real_losses_b", 0)) + 1

    if can_a:
        count_win_a = bool(win_a or white_protection_win)
        session["mirror_wins_a"] = int(session.get("mirror_wins_a", 0)) + (1 if count_win_a else 0)
        session["mirror_losses_a"] = int(session.get("mirror_losses_a", 0)) + (0 if count_win_a else 1)
    if can_b:
        count_win_b = bool(win_b or white_protection_win)
        session["mirror_wins_b"] = int(session.get("mirror_wins_b", 0)) + (1 if count_win_b else 0)
        session["mirror_losses_b"] = int(session.get("mirror_losses_b", 0)) + (0 if count_win_b else 1)
    if actual == "W":
        session["mirror_whites"] = int(session.get("mirror_whites", 0)) + 1
        if protection_enabled and can_a:
            session["mirror_white_protection_hits_a"] = int(
                session.get("mirror_white_protection_hits_a", 0)
            ) + 1
        if protection_enabled and can_b:
            session["mirror_white_protection_hits_b"] = int(
                session.get("mirror_white_protection_hits_b", 0)
            ) + 1

    ops = list(session.get("mirror_operations", []) or [])
    ops.insert(0, {
        "data_hora": rdt,
        "rodada_id": rid,
        "sinal": signal,
        ...
        "resultado": actual,
        "a_resultado": (
            "WIN_BRANCO" if white_protection_win
            else ("WIN" if win_a else "LOSS")
        ),
        "b_resultado": (
            "WIN_BRANCO" if white_protection_win
            else ("WIN" if win_b else "LOSS")
        ),
        "stake": round(stake, 2),
        "white_protection_enabled": protection_enabled,
        "white_protection_stake": round(protection_stake, 2),
        "white_protection_payout": round(protection_payout, 2),
        "white_protection_hit": bool(protection_enabled and actual == "W"),
        "a_executed": bool(can_a),
        "b_executed": bool(can_b),
        "a_liquido_rodada": round(
            (
                (-total_stake)
                + (protection_stake * protection_payout if white_protection_win else 0.0)
                + (stake * 2.0 if win_a else 0.0)
            ) if can_a else 0.0,
            2
        ),
        "b_liquido_rodada": round(
            (
                (-total_stake)
                + (protection_stake * protection_payout if white_protection_win else 0.0)
                + (stake * 2.0 if win_b else 0.0)
            ) if can_b else 0.0,
            2
        ),
        "bank_a": round(bank_a, 2),
        "bank_b": round(bank_b, 2)
    })
    session["mirror_operations"] = ops[:100]
    session["mirror_last_round_key"] = key
    session["mirror_updated_at"] = agora_brasilia()
    return session


def demo_espelho(demo_id, enabled):
    demo_id = str(demo_id or "principal")
    with LOCK:
    ...
    sessao["mirror_enabled"] = enabled
    if enabled:
        base_bank = float(sessao.get("bank", sessao.get("initial_bank", 1000.0)))
        sessao["mirror_initial_bank"] = base_bank
        sessao["mirror_bank_a"] = base_bank
        sessao["mirror_bank_b"] = base_bank
        sessao["mirror_entries"] = 0
        sessao["mirror_wins_a"] = 0
        sessao["mirror_losses_a"] = 0
        sessao["mirror_wins_b"] = 0
        sessao["mirror_losses_b"] = 0
        sessao["mirror_whites"] = 0
        sessao["mirror_white_protection_hits_a"] = 0
        sessao["mirror_white_protection_hits_b"] = 0
        sessao["mirror_operations"] = []
        sessao["mirror_activation_round_id"] = str(last.get("id", ""))
        sessao["mirror_activation_round_data_hora"] = str(last.get("data_hora", ""))
        sessao["mirror_last_round_key"] = str(last.get("id", "")) or str(last.get("data_hora", ""))
        sessao["mirror_started_at"] = agora_brasilia()
...
def _demo_processar_sessao(session, rodada, previous_colors):
    ...
    # V58.16 — REGRA ABSOLUTA DA DEMO:
    # uma rodada anterior (ou igual) ao instante/rodada de ativação NUNCA
    # pode gerar sinal, WIN, LOSS, Gale ou alterar banca.
    activation_time = str(session.get("activation_round_data_hora", "")).strip()
    activation_epoch = _epoch_brasilia(activation_time) if activation_time else 0.0
    rodada_epoch = momento_efetivo_epoch(rodada)
    if activation_epoch > 0.0 and rodada_epoch > 0.0:
        if rodada_epoch <= activation_epoch + 0.001:
            return session
    ...
    session = _demo_processar_espelho(session, rodada, previous_colors)

    # V58.19: META remodela somente com dados ANTERIORES à rodada atual.
    if str(strategy.get("type", "single")) == "meta_adaptive":
        interval = max(1, int(strategy.get("meta_interval", 20)))
        since = int(session.get("meta_since_remodel", 0))
        if since >= interval:
            try:
                current = _meta_recent_strategy(
                    previous_colors,
                    int(strategy.get("meta_analysis_window", 100)),
                    str(strategy.get("meta_objective", "profit")),
                    int(strategy.get("meta_min_train_entries", 2))
                )
                if isinstance(current, dict) and current:
                    strategy["current_strategy"] = current
                    strategy["meta_last_remodel_at"] = agora_brasilia()
                    strategy["meta_remodel_count"] = int(strategy.get("meta_remodel_count", 0)) + 1
                    session["strategy"] = strategy
                    session["meta_since_remodel"] = 0
                    since = 0
            except Exception as exc:
                session["meta_last_error"] = str(exc)
    ...
    if not cycle_active:
        if signal:
            signal_every = max(1, int(strategy.get("signal_entry_every", 1)))
            signal_counter = int(session.get("entry_signal_counter", 0)) + 1
            session["entry_signal_counter"] = signal_counter
            take_signal = signal_every <= 1 or (signal_counter % signal_every) == 0
            session["last_entry_signal_taken"] = bool(take_signal)
            if not take_signal:
                session["entry_signals_skipped"] = int(session.get("entry_signals_skipped", 0)) + 1
            else:
                cycle_active = True
                current_gale = 0
                cycle_target = signal
    ...
    if should_bet:
        ...
        bank = float(session.get("bank", 0.0))
        protection_enabled = (
            bool(session.get("white_protection_enabled", False))
            and not is_white
            and target in ("R", "B")
        )
        protection_stake = (
        ...
        total_required = stake + protection_stake

        if bank < total_required:
            session["losses"] = int(session.get("losses", 0)) + 1
            session["stop_reason"] = (
                "BANCA INSUFICIENTE PARA G%d • precisava R$ %.2f"
                % (current_gale, total_required)
            )
            ...
            if protection_enabled:
                session["white_protection_total_bets"] = int(
                    session.get("white_protection_total_bets", 0)
                ) + 1
                session["white_protection_result"] = float(
                    session.get("white_protection_result", 0.0)
                ) - protection_stake
            session["bank"] = bank
            ...
            actual = str((rodada or {}).get("cor", ""))
            protection_return = 0.0
            session["last_white_protection_enabled"] = protection_enabled
            session["last_white_protection_stake"] = protection_stake
            session["last_white_protection_return"] = 0.0
            session["last_operation_net"] = -total_required

            if protection_enabled and actual == "W":
                protection_return = protection_stake * protection_payout
                bank += protection_return
                session["bank"] = bank
                session["white_protection_hits"] = int(
                    session.get("white_protection_hits", 0)
                ) + 1
                session["white_protection_result"] = float(
                    session.get("white_protection_result", 0.0)
                ) + protection_return
                session["last_white_protection_return"] = protection_return
                session["last_operation_net"] = protection_return - total_required
            ...
            if actual == target:
                ...
                session["last_operation_net"] = (stake * multiplicador) + protection_return - total_required
                _demo_operacao(
                ...
                # V58.24: o alvo principal pode perder e, simultaneamente, o branco
                # protegido pode pagar. O histórico passa a mostrar o resultado
                # financeiro real da rodada, sem apagar a perda da entrada principal.
                net_attempt = protection_return - total_required
                session["last_operation_net"] = net_attempt
                protection_hit = bool(protection_enabled and actual == "W")
                kind = "LOSS_TENTATIVA"
                if protection_hit:
                    kind = "WIN_PROTECAO_BRANCO" if net_attempt >= 0.0 else "LOSS_COM_PROTECAO_BRANCO"
                _demo_operacao(
                    session, rodada, kind, target, stake, current_gale
                )
                if current_gale >= gale_max:
                    # No G0 (e no último Gale), o placar financeiro respeita a
                    # proteção: se ela cobriu principal+proteção, conta WIN financeiro.
                    financial_win = bool(protection_hit and net_attempt >= 0.0)
                    if financial_win:
                        session["wins"] = int(session.get("wins", 0)) + 1
                    else:
                        session["losses"] = int(session.get("losses", 0)) + 1
                    recent = list(session.get("recent_results", []) or [])
                    recent.append(financial_win)
                    session["recent_results"] = recent[-500:]
                    cycle_active = False
                    current_gale = 0
                    cycle_target = ""
            ...
            session["bank"] = bank
            session["peak_bank"] = max(float(session.get("peak_bank", bank)), bank)
    ...
    if str((session.get("strategy") or {}).get("type", "")) == "meta_adaptive":
        session["meta_since_remodel"] = int(session.get("meta_since_remodel", 0)) + 1
...
def demo_iniciar(payload):
    ...
    # V58.17 — proteção anti-reset.
    # O app pode chamar /demo/iniciar novamente ao reconectar/reabrir.
    # Isso NÃO pode recriar uma Demo já existente e apagar banca/placar/operações.
    # Uma sessão existente só é substituída quando o cliente pedir explicitamente
    # force_reset=true (ou após usar /demo/resetar).
    force_reset = bool(payload.get("force_reset", False))
    with LOCK:
        existente = dict(ESTADO.get("demo_contas", {}).get(demo_id, {}) or {})
        base = ordenar_rodadas_canonicas(list(ESTADO.get("rodadas", [])))
        last = dict(base[-1]) if base else {}

    if existente and not force_reset:
        # Regrava no PostgreSQL como autocura caso o estado tenha vindo da memória,
        # mas preserva integralmente a sessão existente.
        if postgres_configurado():
            postgres_demo_salvar(demo_id, existente)
        existente["reused_existing_session"] = True
        existente["anti_reset_v5817"] = True
        return existente
    ...
    sessao = {
        ...
        "white_protection_enabled": bool(payload.get("white_protection_enabled", strategy.get("white_protection_enabled", False))),
        "white_protection_stake": max(0.0, float(payload.get("white_protection_stake", strategy.get("white_protection_stake", 1.0)))),
        "white_protection_payout": max(1.0, float(payload.get("white_protection_payout", strategy.get("white_protection_payout", 14.0)))),
        "white_protection_total_bets": 0,
        "white_protection_hits": 0,
        "white_protection_result": 0.0,
        "mirror_enabled": bool(payload.get("mirror_enabled", False)),
        "mirror_initial_bank": initial_bank,
        "mirror_bank_a": initial_bank,
        "mirror_bank_b": initial_bank,
        "mirror_entries": 0,
        "mirror_wins_a": 0,
        "mirror_losses_a": 0,
        "mirror_wins_b": 0,
        "mirror_losses_b": 0,
        "mirror_whites": 0,
        "mirror_white_protection_hits_a": 0,
        "mirror_white_protection_hits_b": 0,
        "mirror_operations": [],
        "mirror_activation_round_id": str(last.get("id", "")),
        "mirror_activation_round_data_hora": str(last.get("data_hora", "")),
        "mirror_last_round_key": str(last.get("id", "")) or str(last.get("data_hora", "")),
        "mirror_started_at": agora_brasilia(),
        ...
        "cycle_active": False,
        "cycle_target": "",
        "entry_signal_counter": 0,
        "entry_signals_skipped": 0,
        "last_entry_signal_taken": False,
        ...
        "activation_server_time": agora_brasilia(),
        "operations": [],
        "recent_results": [],
        "stop_reason": "",
...
def demo_status(demo_id="principal", compact=False):
    with LOCK:
        sessao = dict(
    ...
    # V58.14: status da Demo não reordena até 30.000 rodadas a cada poll.
    # A base já é mantida em ordem pelo coletor; basta consultar a última.
    with LOCK:
        base_ref = ESTADO.get("rodadas", [])
        server_last = dict(base_ref[-1]) if isinstance(base_ref, list) and base_ref else {}
    ...
    if compact:
        # Evita enviar megabytes a cada atualização da tela.
        # Esses campos são internos ou muito grandes e não são necessários
        # para atualizar placar/sinal/banca ao vivo.
        out.pop("processed_round_keys", None)
        out.pop("recent_results", None)

        ops = list(out.get("operations", []) or [])
        out["operations"] = ops[:40]
        mirror_ops = list(out.get("mirror_operations", []) or [])
        out["mirror_operations"] = mirror_ops[:20]

        strategy = out.get("strategy", {})
        if isinstance(strategy, dict):
            strategy_small = dict(strategy)
            comps = strategy_small.pop("components", None)
            if isinstance(comps, list):
                strategy_small["component_count"] = len(comps)
            out["strategy"] = strategy_small
...
def demo_alterar_estrategia(payload):
    ...
    with LOCK:
        ...
        sessao["activation_server_time"] = agora_brasilia()
        sessao["processed_round_keys"] = []
        sessao["meta_since_remodel"] = 0
...
def demo_protecao_branco(demo_id="principal", enabled=False, stake=None):
    demo_id = str(demo_id or "principal")
    with LOCK:
        sessao = dict(ESTADO.get("demo_contas", {}).get(demo_id, {}) or {})
    if not sessao:
    ...
    sessao["white_protection_enabled"] = bool(enabled)
    if stake is not None:
        sessao["white_protection_stake"] = max(0.0, float(stake))
    sessao["white_protection_payout"] = max(
        1.0, float(sessao.get("white_protection_payout", 14.0))
    )
    sessao["updated_at"] = agora_brasilia()
...
def adicionar_rodadas_em_lote(rodadas_novas):
    ...
    if adicionadas > 0:
        ...
        # V58.16: IMPORTAÇÃO/RECONCILIAÇÃO HISTÓRICA JAMAIS ALIMENTA A DEMO.
...
# V58.37 — reparo MULTIFONTE, aditivo e não destrutivo de lacunas históricas.
# Nunca substitui ESTADO["rodadas"], nunca apaga registros e nunca cria resultado.
_REPARO_LACUNAS_LOCK = threading.Lock()


def _listar_lacunas_historico_completo(limite_gap=None):
    """Retorna todas as lacunas temporais da base atual, sem alterar a base."""
    if limite_gap is None:
        limite_gap = LIMITE_LACUNA_HISTORICO_SEGUNDOS
    limite_gap = max(45, int(limite_gap))
...
def _timestamp_em_alguma_lacuna(t, intervalos):
    try:
        t = float(t)
    except Exception:
        return False
    if t <= 0:
        return False
    return any(float(a) < t < float(b) for a, b in intervalos)


def _rodada_json_blaze(item):
    """Converte um registro JSON oficial/compatível da Blaze para o formato interno."""
    if not isinstance(item, dict):
        return None
    cor = _cor_ws_para_rbW(item.get("color"))
    if not cor:
        # alguns mirrors devolvem nomes de cor
        cor = normalizar_cor(item.get("color", item.get("cor", "")))
    if cor not in ("R", "B", "W"):
        return None

    stamp = item.get("created_at") or item.get("updated_at") or item.get("rolled_at") or item.get("date")
    dt_utc = _parse_iso_utc_para_datetime(stamp)
    if dt_utc is not None:
        brasilia = timezone(timedelta(hours=-3))
        data_hora = dt_utc.astimezone(brasilia).strftime("%d/%m/%Y %H:%M:%S")
    else:
        bruto = str(stamp or item.get("data_hora", "")).strip()
        data_hora = ""
        for fmt in ("%d/%m/%Y %H:%M:%S", "%Y-%m-%d %H:%M:%S"):
            try:
                data_hora = datetime.strptime(bruto, fmt).strftime("%d/%m/%Y %H:%M:%S")
                break
            except Exception:
                pass
        if not data_hora:
    ...
    try:
        numero = int(item.get("roll")) if item.get("roll") is not None else None
    except Exception:
        numero = None
    return {
        ...
        "cor": cor,
        "data_hora": data_hora,
        "origem": "reparo_blaze_json",
    }


def _extrair_registros_json_blaze(obj):
    if isinstance(obj, list):
        return obj
    if not isinstance(obj, dict):
        return []
    for chave in ("records", "items", "data", "results", "rodadas"):
        valor = obj.get(chave)
        if isinstance(valor, list):
            return valor
        if isinstance(valor, dict):
            for sub in ("records", "items", "data", "results"):
                if isinstance(valor.get(sub), list):
                    return valor.get(sub)
    return []


def _http_json(url, timeout=18):
    req = Request(url, headers={
        "User-Agent": "Mozilla/5.0 DoubleBlazeIA/58.37",
...
def _reparo_blaze_json_paginado(intervalos, max_paginas=260):
    ...
    if not intervalos:
        return [], {"ok": False, "erro": "sem intervalos"}
    minimo = min(a for a, _ in intervalos)
    maximo = max(b for _, b in intervalos)
    endpoints = [
        "https://blaze.bet.br/api/roulette_games/history?page={page}",
        "https://blaze.com/api/roulette_games/history?page={page}",
        "https://blaze.bet.br/api/singleplayer-originals/originals/roulette_games/history/1?page={page}",
    ]

    erros = []
    for modelo in endpoints:
        try:
            obj, status = _http_json(modelo.format(page=1))
            regs = _extrair_registros_json_blaze(obj)
            if not regs:
                raise RuntimeError("resposta sem registros")
        except Exception as exc:
            erros.append({"endpoint": modelo, "erro": str(exc)[:220]})
            continue
        ...
        for pagina in range(1, max(1, int(max_paginas)) + 1):
            try:
                if pagina == 1:
                    ...
                    atual, st = _http_json(modelo.format(page=pagina))
                itens = _extrair_registros_json_blaze(atual)
                if not itens:
                    break
                paginas += 1
                epochs_pagina = []
                for item in itens:
                    ...
                    if not rodada:
                        continue
                    t = momento_efetivo_epoch(rodada)
                    if t > 0:
                        epochs_pagina.append(t)
                        if _timestamp_em_alguma_lacuna(t, intervalos):
                            encontrados.append(rodada)
                if epochs_pagina:
                    pmin, pmax = min(epochs_pagina), max(epochs_pagina)
                    mais_antigo = pmin if mais_antigo is None else min(mais_antigo, pmin)
                    mais_novo = pmax if mais_novo is None else max(mais_novo, pmax)
                    # já atravessou o começo de todas as lacunas: não precisa pedir páginas mais antigas.
                    if pmin <= minimo - 90:
                        break
                # proteção contra endpoint que repete sempre a mesma página
                if pagina >= 3 and mais_antigo is not None and mais_antigo > maximo + 86400:
                    break
                time.sleep(0.05)
            except Exception as exc:
                erros.append({"endpoint": modelo, "pagina": pagina, "erro": str(exc)[:220]})
                break
        ...
        return encontrados, {
            ...
            "endpoint": modelo,
            "paginas": paginas,
            "encontradas_em_lacunas": len(encontrados),
            "mais_antigo_epoch": mais_antigo,
            "mais_novo_epoch": mais_novo,
            "erros": erros[-5:],
        }

    return [], {"ok": False, "erro": "nenhum endpoint JSON histórico acessível", "tentativas": erros}


def _bestblaze_form_com_opcoes(html, base_url="https://bestblaze.com.br/doubleRodadas"):
    form = detectar_formulario_periodo_bestblaze(html, base_url)
    if not form:
        return None, {}
    selects = {}
    forms = re.findall(r"(?is)<form\b([^>]*)>(.*?)</form>", html or "")
    for _attrs, corpo in forms:
        if form.get("campo_inicial", "") not in corpo or form.get("campo_final", "") not in corpo:
            continue
        for attrs_select, corpo_select in re.findall(r"(?is)<select\b([^>]*)>(.*?)</select>", corpo):
            nome = _atributo_html(attrs_select, "name", "")
            if not nome:
                continue
            opcoes = []
            for attrs_opt, texto_opt in re.findall(r"(?is)<option\b([^>]*)>(.*?)</option>", corpo_select or ""):
                valor = _atributo_html(attrs_opt, "value", "")
                texto = re.sub(r"<[^>]+>", " ", texto_opt or "")
                texto = re.sub(r"\s+", " ", unescape(texto)).strip()
                opcoes.append({"value": str(valor), "text": texto})
            selects[nome] = opcoes
        break
    return form, selects


def _bestblaze_escolher_campo_hora(form, selects):
    candidatos = []
    for nome, opcoes in (selects or {}).items():
        baixo = str(nome).lower()
        score = 100 if ("hora" in baixo or "hour" in baixo) else 0
        if len(opcoes) >= 20:
            score += 20
        if score:
...
def _bestblaze_varrer_opcoes_dia(data_alvo, intervalos):
    """Não adivinha hora: envia CADA value real do select e confia apenas nos timestamps retornados."""
    base_url = "https://bestblaze.com.br/doubleRodadas"
    opener, jar = criar_sessao_bestblaze()
    pagina_inicial, status_get = abrir_com_sessao_bestblaze(opener, base_url)
    form, selects = _bestblaze_form_com_opcoes(pagina_inicial, base_url)
    if not form:
        raise RuntimeError("formulário de período não identificado")
    campo_hora = _bestblaze_escolher_campo_hora(form, selects)
    if not campo_hora:
        raise RuntimeError("select de hora não identificado")

    valores = []
    for opt in selects.get(campo_hora, []):
        valor = str(opt.get("value", "")).strip()
        if valor not in valores:
            valores.append(valor)
    if not valores:
        raise RuntimeError("select de hora sem opções")

    def fd(dt, tipo):
        return dt.strftime("%Y-%m-%dT%H:%M") if tipo == "datetime-local" else dt.strftime("%Y-%m-%d")

    inicio = data_alvo.replace(hour=0, minute=0, second=0, microsecond=0)
    fim = data_alvo.replace(hour=23, minute=59, second=59, microsecond=0)
    candidatos = []
    diag = []
    ...
    for valor in valores:
        ...
        payload[campo_hora] = valor
        payload = {str(k): "" if v is None else str(v) for k, v in payload.items() if str(k).strip()}
        encoded_text = urlencode(payload)
        encoded = encoded_text.encode("utf-8")
        try:
            if form["method"] == "POST":
                html, st = abrir_com_sessao_bestblaze(opener, form["action"], data=encoded, referer=base_url)
            else:
                sep = "&" if "?" in form["action"] else "?"
                html, st = abrir_com_sessao_bestblaze(opener, form["action"] + sep + encoded_text, referer=base_url)
            reconhecidas = extrair_bestblaze_historico_html(html)
            aceitas = 0
            horas = set()
            for rodada in reconhecidas:
                if str(rodada.get("cor", "")) not in ("R", "B", "W"):
                    continue
                try:
                    dt = datetime.strptime(str(rodada.get("data_hora", "")), "%d/%m/%Y %H:%M:%S")
                    horas.add(dt.hour)
                except Exception:
                    pass
                t = momento_efetivo_epoch(rodada)
                if _timestamp_em_alguma_lacuna(t, intervalos):
                    rr = dict(rodada)
                    rr["origem"] = "reparo_bestblaze_exaustivo"
                    candidatos.append(rr)
                    aceitas += 1
            diag.append({"value": valor, "status": int(st), "retornadas": len(reconhecidas), "horas": sorted(horas), "aceitas": aceitas})
        except Exception as exc:
            diag.append({"value": valor, "erro": str(exc)[:180]})
        time.sleep(0.04)

    return candidatos, {"status_get": int(status_get), "campo_hora": campo_hora, "opcoes_testadas": len(valores), "tentativas": diag}


def _deduplicar_candidatos_reparo(candidatos, intervalos):
    mapa = {}
    for rodada in candidatos or []:
        if not isinstance(rodada, dict) or str(rodada.get("cor", "")) not in ("R", "B", "W"):
            continue
        t = momento_efetivo_epoch(rodada)
        if not _timestamp_em_alguma_lacuna(t, intervalos):
            continue
        chave = str(rodada.get("data_hora", ""))
        if not chave:
            continue
        anterior = mapa.get(chave)
        if anterior is None or str(rodada.get("origem", "")).startswith("reparo_blaze_json"):
            mapa[chave] = rodada
    return ordenar_rodadas_canonicas(list(mapa.values()))


def reparar_lacunas_reais_seguro(max_lacunas=500, max_dias=31):
    """V58.37: Blaze JSON paginado + varredura exaustiva BestBlaze; sempre aditivo."""
    max_lacunas = max(1, min(int(max_lacunas), 500))
    max_dias = max(1, min(int(max_dias), 31))
    if not _REPARO_LACUNAS_LOCK.acquire(blocking=False):
        return {"ok": True, "ocupado": True, "mensagem": "reparo já em andamento"}

    try:
        antes = diagnosticar_continuidade_historico()
        lacunas_todas = _listar_lacunas_historico_completo()
        if not lacunas_todas:
            resumo = {"status": "sem_lacunas", "ultima_execucao": agora_brasilia(), "adicionadas": 0, "lacunas_antes": 0, "lacunas_depois": 0, "erro": ""}
            with LOCK:
                ESTADO["reparo_lacunas"] = resumo
                salvar_json(BANCO, ESTADO)
        ...
        candidatos = []
        json_cands, diag_json = _reparo_blaze_json_paginado(intervalos, max_paginas=max(40, int(os.getenv("REPARO_BLAZE_MAX_PAGINAS", "260"))))
        candidatos.extend(json_cands)

        diag_best = []
        for idx, d in enumerate(dias, 1):
            alvo = datetime.strptime(d, "%Y-%m-%d")
            with LOCK:
                ESTADO["reparo_lacunas"] = {"status": "executando", "etapa": "bestblaze_varredura_exaustiva", "progresso": f"{idx}/{len(dias)}", "dia_atual": alvo.strftime("%d/%m/%Y"), "lacunas_antes": int(antes.get("lacunas", 0)), "candidatas": len(candidatos), "erro": ""}
            try:
                cands, dg = _bestblaze_varrer_opcoes_dia(alvo, intervalos)
                candidatos.extend(cands)
                diag_best.append({"dia": alvo.strftime("%d/%m/%Y"), "ok": True, "candidatas": len(cands), "opcoes_testadas": dg.get("opcoes_testadas", 0), "tentativas": dg.get("tentativas", [])})
            except Exception as exc:
                diag_best.append({"dia": alvo.strftime("%d/%m/%Y"), "ok": False, "erro": str(exc)[:300]})

        unicos = _deduplicar_candidatos_reparo(candidatos, intervalos)
        resultado_lote = adicionar_rodadas_em_lote(unicos) if unicos else {"recebidas": 0, "adicionadas": 0, "duplicadas": 0}
        depois = diagnosticar_continuidade_historico()
        resumo = {
            "status": "concluido",
            "ultima_execucao": agora_brasilia(),
            "adicionadas": int(resultado_lote.get("adicionadas", 0)),
            "candidatas": len(unicos),
            "lacunas_antes": int(antes.get("lacunas", 0)),
            "lacunas_depois": int(depois.get("lacunas", 0)),
            "total_antes": int(antes.get("total_bruto", 0)),
            "total_depois": int(depois.get("total_bruto", 0)),
            "fonte_blaze_json": diag_json,
            "fonte_bestblaze": diag_best,
            "erro": "",
        }
        with LOCK:
            ESTADO["reparo_lacunas"] = resumo
        ...
        with LOCK:
            ESTADO["reparo_lacunas"] = {"status": "erro_sem_alteracao_destrutiva", "ultima_execucao": agora_brasilia(), "adicionadas": 0, "erro": str(exc)}
            salvar_json(BANCO, ESTADO)
        return {"ok": False, "erro": str(exc), "adicionadas": 0}
    finally:
        _REPARO_LACUNAS_LOCK.release()

def disparar_reparo_lacunas_background(origem="manual"):
    """Dispara uma tentativa sem bloquear HTTP; nunca executa duas em paralelo."""
    with LOCK:
        atual = dict(ESTADO.get("reparo_lacunas", {}) or {})
        if atual.get("status") in ("agendado", "executando"):
            return {"ok": True, "iniciado": False, "mensagem": "reparo já agendado/em execução", "reparo": atual}
        ESTADO["reparo_lacunas"] = {
            "status": "agendado",
            "origem": str(origem),
            "agendado_em": agora_brasilia(),
            "adicionadas": 0,
            "erro": "",
        }
        salvar_json(BANCO, ESTADO)
    ...
    def _run():
        with LOCK:
            ESTADO["reparo_lacunas"] = {
                "status": "executando",
                "origem": str(origem),
                "iniciado_em": agora_brasilia(),
                "adicionadas": 0,
                "erro": "",
            }
            salvar_json(BANCO, ESTADO)
        resultado = reparar_lacunas_reais_seguro(
            max_lacunas=max(1, int(os.getenv("REPARO_LACUNAS_MAX", "500"))),
            max_dias=max(1, int(os.getenv("REPARO_LACUNAS_MAX_DIAS", "31"))),
        )
        # reparar_lacunas_reais_seguro já grava o resumo final.
        return resultado

    threading.Thread(target=_run, daemon=True, name="reparo-lacunas-manual").start()
    return {"ok": True, "iniciado": True, "mensagem": "reparo agendado"}


def worker_reparo_inicial():
    """
    V58.35 — reparo seguro, exaustivo, auditável, aditivo e não destrutivo.
    Aguarda o PostgreSQL/coletores estabilizarem e tenta preencher somente
    ...
    time.sleep(25)
    intervalo = max(300, int(os.getenv("REPARO_LACUNAS_INTERVALO_SEGUNDOS", "900")))
    max_lacunas = max(1, int(os.getenv("REPARO_LACUNAS_MAX", "500")))
    max_dias = max(1, int(os.getenv("REPARO_LACUNAS_MAX_DIAS", "31")))

    while True:
        try:
            resultado = reparar_lacunas_reais_seguro(max_lacunas=max_lacunas, max_dias=max_dias)
            print(
                "V58.33 reparo seguro:",
                "adicionadas=", resultado.get("adicionadas", 0),
                "lacunas_antes=", resultado.get("lacunas_antes", "?"),
                "lacunas_depois=", resultado.get("lacunas_depois", "?"),
                flush=True,
            )
        except Exception as exc:
            print("V58.33 reparo seguro erro:", exc, flush=True)
...
def worker_bootstrap_1500():
    """
    V58.15 — DESATIVADO.
    Em produção, deploy/restart não reconstrói nem substitui a base histórica.
    A persistência real depende do PostgreSQL (DATABASE_URL).
    """
    with LOCK:
        ESTADO["bootstrap_1500"] = {
            "status": "desativado_persistencia",
            "tentativa": agora_brasilia(),
            "sucesso": "",
            "erro": "",
            "quantidade": len(ESTADO.get("rodadas", [])),
            "motivo": "nao_reconstruir_em_deploy"
        }
    return
...
def _optimizer_simulate_mask(colors, signal_mask, targets, cfg, gale, mode):
    ...
    def place_bet(idx, target, g, exposure):
        ...
        wp = white_protection_stake if white_protection_enabled else 0.0
        total_stake = stake + wp
        if bank < total_stake:
            bankrupt_stops += 1
            losses += 1
...
def _optimizer_numba_core(
    colors, positions, targets, gale, mode, stake_base, initial_bank,
    white_protection_enabled, white_protection_stake, white_protection_payout,
    signal_entry_every
):
    n = colors.shape[0]
    m = positions.shape[0]
    ...
    p = 0
    entry_signal_counter = 0
    signal_entry_every = max(1, int(signal_entry_every))
    while p < m:
        idx = int(positions[p])
        ...
        # V58.32: filtro de entrada por ordinal de sinal.
        # 1 = entra em todos; 2 = 2º, 4º, 6º...; 4 = 4º, 8º, 12º...
        # O contador vale apenas para NOVAS entradas; Gale mantém sua regra original.
        entry_signal_counter += 1
        if signal_entry_every > 1 and (entry_signal_counter % signal_entry_every) != 0:
            p += 1
            continue
        ...
        for g in range(gale + 1):
            ...
            # Proteção simultânea: cada aposta R/P também aposta wp no branco.
            # O lucro líquido inclui o custo em TODAS as entradas. O pagamento 14x é RETORNO TOTAL: R$1 no branco acertando credita R$14.
            if white_protection_enabled and actual == 3:
                bank += wp * white_protection_payout
...
def _optimizer_excluded_pairs(blocked_signatures, gale, mode, contrary,
                              white_protection_enabled=None,
                              white_protection_stake=None):
    """
    V58.28: bloqueia exatamente configurações que continuam em SALVAS.
    A assinatura ensemble contém assinaturas internas com '|g...'; por isso
    o sufixo é interpretado da DIREITA para a ESQUERDA.

    Se a melhor estiver salva, o sweep a ignora e naturalmente escolhe
    a 2ª melhor; se esta também estiver salva, a 3ª, e assim por diante.
    Ao excluir de SALVAS, a assinatura deixa de chegar em blocked_signatures
    e volta a ser elegível.
    """
    counts = []
    consensuses = []
    inv_target = bool(contrary)
    wp_target = None if white_protection_enabled is None else bool(white_protection_enabled)
    ws_target = None if white_protection_stake is None else round(float(white_protection_stake), 2)

    for raw in blocked_signatures or []:
        sig = str(raw or "").strip()
        if not sig:
            continue

        # SINGLE: aceita assinatura antiga e atual.
        m_single = re.match(
            r"^single\|.*\|g(\d+)\|m(\d+)(?:\|se\d+)?\|inv(true|false)"
            r"(?:\|wp(true|false)\|w([0-9.]+))?$",
            sig, flags=re.IGNORECASE
        )
        if m_single:
            sg, sm = int(m_single.group(1)), int(m_single.group(2))
            sinv = m_single.group(3).lower() == "true"
            swp = None if m_single.group(4) is None else (m_single.group(4).lower() == "true")
            sws = None if m_single.group(5) is None else round(float(m_single.group(5)), 2)
            wp_ok = (wp_target is None or swp is None or swp == wp_target)
            ws_ok = (ws_target is None or sws is None or sws == ws_target)
            if sg == int(gale) and sm == int(mode) and sinv == inv_target and wp_ok and ws_ok:
                counts.append(1)
                consensuses.append(-1)
            continue

        # ENSEMBLE: ancora o sufixo final; o corpo pode conter vários "|g...".
        m_ens = re.match(
            r"^ensemble\|(.*)\|g(\d+)\|m(\d+)\|c(\d+)(?:\|se\d+)?\|inv(true|false)"
            r"(?:\|wp(true|false)\|w([0-9.]+))?$",
            sig, flags=re.IGNORECASE
        )
        if not m_ens:
        ...
        body = str(m_ens.group(1))
        sg, sm = int(m_ens.group(2)), int(m_ens.group(3))
        scon = int(m_ens.group(4))
        sinv = m_ens.group(5).lower() == "true"
        swp = None if m_ens.group(6) is None else (m_ens.group(6).lower() == "true")
        sws = None if m_ens.group(7) is None else round(float(m_ens.group(7)), 2)

        wp_ok = (wp_target is None or swp is None or swp == wp_target)
        ws_ok = (ws_target is None or sws is None or sws == ws_target)
        if sg == int(gale) and sm == int(mode) and sinv == inv_target and wp_ok and ws_ok:
            # Cada componente começa por "single|". Isto é mais robusto que
            # contar ';' caso a assinatura interna evolua.
            count = body.count("single|")
            if count <= 0 and body:
                count = body.count(";") + 1
            if count > 0:
                counts.append(int(count))
                consensuses.append(int(scon))

    unique = []
    seen = set()
    for c, con in zip(counts, consensuses):
        key = (int(c), int(con))
        if key not in seen:
            seen.add(key)
            unique.append(key)
    ...
    return (
        np.asarray([x[0] for x in unique], dtype=np.int32),
        np.asarray([x[1] for x in unique], dtype=np.int16)
    )
...
def _optimizer_sweep_chunk_numba(
    ...
    financial_target_mode,
    signal_entry_every,
    min_entries_enabled,
    min_entries,
    min_signal_pct_enabled,
    min_signal_pct,
    dd_enabled,
    dd_limit,
    white_protection_enabled,
    white_protection_stake,
    white_protection_payout,
    skip_counts,
    skip_consensuses
):
    """
    ...
    for count0 in range(start_count, end_count):
        ...
        for ci in range(consensuses.shape[0]):
            ...
            # V58.20: configuração já existente em SALVAS é inelegível.
            skip_this = False
            for si in range(skip_counts.shape[0]):
                if int(skip_counts[si]) == count:
                    skip_con = int(skip_consensuses[si])
                    if skip_con < 0 or skip_con == con:
                        skip_this = True
                        break
            if skip_this:
                continue
            ...
            # V58.31: filtro financeiro por magnitude do objetivo dentro da janela.
            # Lucro usa profit diretamente; prejuízo usa a magnitude positiva (-profit).
            if financial_filter_enabled:
                financial_metric = -profit if objective_mode == 1 else profit
                if financial_target_mode == 1:
                    # ATÉ O VALOR
                    if financial_metric - 0.0001 > financial_target_value:
                        passes = False
                else:
                    # PELO MENOS O VALOR
                    if financial_metric + 0.0001 < financial_target_value:
            ...
            # V58.15: para buscar prejuízo com ALTA ATIVIDADE.
            if min_entries_enabled and entries < min_entries:
                passes = False

            signal_pct = (100.0 * entries / n) if n > 0 else 0.0
            if min_signal_pct_enabled and signal_pct + 0.0001 < min_signal_pct:
                passes = False
...
def _optimizer_simulate_numba(colors, signal_positions, targets, cfg, gale, mode, signal_entry_every=1):
    ...
    if (bool(cfg.get("risk_trailing_enabled", False)) or
        ...
        return _optimizer_simulate_sparse(colors, signal_positions, targets, cfg, gale, mode, signal_entry_every)
    ...
    vals = _optimizer_numba_core(
        ...
        bool(cfg.get("white_protection_enabled", False)),
        max(0.0, float(cfg.get("white_protection_stake", 1.0))),
        max(1.0, float(cfg.get("white_protection_payout", 14.0))),
        max(1, int(signal_entry_every))
    )
    (entries, wins, losses, gale_wins, profit, rate, bank, max_dd,
     max_gale_reached, largest_stake, max_cycle_exposure, min_bank,
     bankrupt_stops, peak_bank, bank_at_max_dd, peak_at_max_dd) = vals
...
def _optimizer_simulate_sparse(colors, signal_positions, targets, cfg, gale, mode, signal_entry_every=1):
    """
    ...
    stake_base = float(cfg.get("stake", 10.0))
    initial_bank = float(cfg.get("initial_bank", 1000.0))
    white_protection_enabled = bool(cfg.get("white_protection_enabled", False))
    white_protection_stake = max(0.0, float(cfg.get("white_protection_stake", 1.0)))
    white_protection_payout = max(1.0, float(cfg.get("white_protection_payout", 14.0)))
    bank = initial_bank
    peak = bank
    min_bank = bank
    max_dd = 0.0
...
def _optimizer_strategy_dict(selected, gale, mode, consensus, cfg, contrary, signal_entry_every=1):
    comps = []
    ...
    for key in (
        ...
        "white_protection_enabled", "white_protection_stake",
        "white_protection_payout"
    ):
        if key in cfg:
            st[key] = cfg[key]
...
def _optimizer_select_history_window(colors_raw, history_ids, payload):
    """
    V58.30 — escolhe automaticamente uma janela recente quando history_window=-1.

    A seleção é deliberadamente rápida: compara janelas candidatas pela capacidade
    dos padrões individuais curtos (1..5) de produzir o melhor resultado no mesmo
    objetivo (lucro/prejuízo), usando G0 tradicional. Depois a busca EXATA completa
    roda somente na janela vencedora.

    Isso evita multiplicar o custo do otimizador completo por 6-8 janelas.
    """
    n_all = len(colors_raw)
    requested = int(payload.get("history_window", 0))

    if requested > 0:
        w = max(20, min(n_all, requested))
        return w, "FIXA"

    if requested == 0:
        return n_all, "TODO_HISTORICO"
    ...
    candidates = []
    for x in candidates_raw if isinstance(candidates_raw, list) else []:
        try:
            w = int(x)
        except Exception:
            continue
        if 20 <= w <= n_all and w not in candidates:
            candidates.append(w)
    if n_all not in candidates:
        candidates.append(n_all)
    candidates.sort()

    best_w = n_all
    best_score = float("inf") if objective == "loss" else float("-inf")

    for w in candidates:
        sub = colors_raw[-w:]
        colors = np.fromiter(
            (1 if c == "R" else 2 if c == "B" else 0 for c in sub),
            dtype=np.uint8, count=w
        )
        occ = _optimizer_occurrences(sub)
        local_best = float("inf") if objective == "loss" else float("-inf")

        # Seleção rápida baseada em todos os padrões de comprimento 1..5.
        for plen in range(1, 6):
            for mask in range(1 << plen):
                positions = np.asarray(occ[(plen, mask)], dtype=np.int32)
                if positions.size == 0:
                    continue
                targets = np.zeros(w, dtype=np.uint8)
                for target_code in (1, 2):
                    targets.fill(0)
                    targets[positions] = target_code
                    r = _optimizer_simulate_numba(
                        colors, positions, targets, cfg, 0, 0
                    )
                    score = float(r.get("profit", 0.0))
                    if objective == "loss":
                        if score < local_best:
                            local_best = score
                    else:
        ...
        if objective == "loss":
            if local_best < best_score:
                best_score, best_w = local_best, w
        else:
            if local_best > best_score:
                best_score, best_w = local_best, w

    return int(best_w), "AUTO"




def _optimizer_single_block_signature(item, gale, mode, contrary, cfg, signal_entry_every):
    entry = str(item.get("entry", ""))
    if contrary:
        entry = "B" if entry == "R" else "R"
    pattern = ",".join(str(x) for x in (item.get("pattern", []) or []))
    return "single|%s|%s|g%d|m%d|se%d|inv%s|wp%s|w%.2f" % (
        pattern, entry, int(gale), int(mode), max(1, int(signal_entry_every)),
        "true" if bool(contrary) else "false",
        "true" if bool(cfg.get("white_protection_enabled", False)) else "false",
        float(cfg.get("white_protection_stake", 0.0))
    )


def _optimizer_single_passes_filters(result, n, objective,
                                     min_rate_enabled, min_rate, rate_mode,
                                     financial_filter_enabled, financial_target_value, financial_target_mode,
                                     min_entries_enabled, min_entries,
                                     min_signal_pct_enabled, min_signal_pct,
                                     dd_enabled, dd_limit):
    ...
    if min_rate_enabled:
        if rate_mode == "max":
            if rate - 0.0001 > min_rate:
                return False
        elif rate + 0.0001 < min_rate:
            return False
    if financial_filter_enabled:
        metric = -profit if objective == "loss" else profit
        if financial_target_mode == "at_most":
            if metric - 0.0001 > financial_target_value:
                return False
        elif metric + 0.0001 < financial_target_value:
            return False
    if min_entries_enabled and entries < min_entries:
        return False
    signal_pct = (100.0 * entries / n) if n > 0 else 0.0
...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        selected_window, window_mode = _optimizer_select_history_window(
            colors_raw, history_ids, payload
        )
        selected_window = max(20, min(n_all, int(selected_window)))
        colors_raw = colors_raw[-selected_window:]
        history_ids = history_ids[-selected_window:]
        n = len(colors_raw)
        ...
        strategy_search_mode = str(payload.get("strategy_search_mode", "ensemble")).strip().lower()
        if strategy_search_mode not in ("ensemble", "single"):
            strategy_search_mode = "ensemble"
        ...
        financial_filter_enabled = bool(payload.get("financial_filter_enabled", False))
        financial_target_value = max(0.0, float(payload.get("financial_target_value", 0.0)))
        financial_target_mode = str(payload.get("financial_target_mode", "at_least")).strip().lower()
        if financial_target_mode not in ("at_least", "at_most"):
            financial_target_mode = "at_least"
        financial_target_mode_code = 1 if financial_target_mode == "at_most" else 0

        # V58.32: entra somente a cada N-ésimo sinal de nova entrada.
        # 1 mantém exatamente o comportamento histórico anterior.
        signal_entry_every = max(1, min(1000, int(payload.get("signal_entry_every", 1))))

        min_entries_enabled = bool(payload.get("min_entries_enabled", False))
        min_entries = max(0, int(payload.get("min_entries", 0)))

        min_signal_pct_enabled = bool(payload.get("min_signal_pct_enabled", False))
        min_signal_pct = max(0.0, min(100.0, float(payload.get("min_signal_pct", 0.0))))

        dd_enabled = bool(payload.get("dd_enabled", False))
        dd_limit = float(payload.get("dd_limit", 0.0))

        blocked_signatures_raw = payload.get("blocked_signatures", [])
        blocked_signatures = [
            str(x).strip()
            for x in (blocked_signatures_raw if isinstance(blocked_signatures_raw, list) else [])
            if str(x).strip()
        ]
        ...
        cache_payload = {
            "engine": "v58.38-single-or-ensemble-exact",
            "history": hist_hash,
            "gales": gales, "modes": modes,
            "strategy_search_mode": strategy_search_mode,
            ...
            "rate_mode": rate_mode,
            "objective": objective,
            "financial_filter_enabled": financial_filter_enabled,
            "financial_target_value": financial_target_value,
            "financial_target_mode": financial_target_mode,
            "signal_entry_every": signal_entry_every,
            "min_entries_enabled": min_entries_enabled,
            "min_entries": min_entries,
            "min_signal_pct_enabled": min_signal_pct_enabled,
            "min_signal_pct": min_signal_pct,
            "dd_enabled": dd_enabled,
            "dd_limit": dd_limit,
            "blocked_signatures": sorted(blocked_signatures),
            "history_window_requested": int(payload.get("history_window", 0)),
            "history_window_selected": int(selected_window),
            "history_window_mode": str(window_mode)
        }
        ...
        total_configs = (
            len(gales) * len(modes) * 3000
            if strategy_search_mode == "single"
            else len(gales) * len(modes) * max_count_req * len(consensuses)
        )
        _optimizer_job_update(
            job_id, status="running",
            phase="JANELA %d RODADAS • PREPARANDO MOTOR" % int(selected_window),
        ...
        for gale in gales:
            for mode in modes:
                ...
                ranking_key_raw = {
                    ...
                    "initial_bank": float(cfg.get("initial_bank", 1000.0)),
                    "white_protection_enabled": bool(cfg.get("white_protection_enabled", False)),
                    "white_protection_stake": float(cfg.get("white_protection_stake", 1.0)),
                    "white_protection_payout": float(cfg.get("white_protection_payout", 14.0)),
                    "rate_mode": str(rate_mode),
                    "objective": str(objective),
                    "signal_entry_every": int(signal_entry_every)
                }
                ...
                if ranked is None:
                    ...
                    for idx, cand0 in enumerate(base):
                        ...
                        for entry, target_code in (("R", 1), ("B", 2)):
                            ...
                            targets[positions] = target_code
                            r = _optimizer_simulate_numba(
                                colors, positions, targets, cfg, gale, mode, signal_entry_every
                            )
                ...
                # V58.38: modo ESTRATÉGIA ÚNICA.
                # Testa as 3000 estratégias individuais uma por uma. Não usa
                # prefixo/combinação nem consenso; todos os demais filtros continuam ativos.
                if strategy_search_mode == "single":
                    blocked_set = set(blocked_signatures)
                    for si, cand in enumerate(ranked):
                        sig = _optimizer_single_block_signature(
                            cand, gale, mode, contrary, cfg, signal_entry_every
                        )
                        tested += 1
                        if sig in blocked_set:
                            continue

                        positions = np.asarray(cand["positions"], dtype=np.int32)
                        target_code = 1 if str(cand.get("entry", "R")) == "R" else 2
                        if contrary:
                            target_code = 2 if target_code == 1 else 1
                        targets = np.zeros(n, dtype=np.uint8)
                        if positions.size > 0:
                            targets[positions] = target_code
                        ...
                        if not _optimizer_single_passes_filters(
                            r, n, objective,
                            min_rate_enabled, min_rate, rate_mode,
                            financial_filter_enabled, financial_target_value, financial_target_mode,
                            min_entries_enabled, min_entries,
                            ...
                            if tested % 100 == 0:
                                _optimizer_job_update(
                                    job_id, phase="ESTRATÉGIAS ÚNICAS • %d/3000 • G%d • M%d" % (si + 1, gale, mode),
                                    tested=tested, total=total_configs,
                                    progress=min(0.999, tested / float(max(1, total_configs)))
                                )
                            continue
                        ...
                        better = profit < best_profit if objective == "loss" else profit > best_profit
                        if better or (abs(profit - best_profit) < 1e-9 and max_dd < best_dd):
                            best_profit = profit
                            best_dd = max_dd
                            best_strategy = _optimizer_strategy_dict(
                                [cand], gale, mode, 50, cfg, contrary, signal_entry_every
                            )
                            best_result = dict(r)
                        ...
                        if tested % 100 == 0:
                            ...
                            _optimizer_job_update(
                                job_id,
                                phase="ESTRATÉGIAS ÚNICAS • %d/3000 • G%d • M%d" % (si + 1, gale, mode),
                                tested=tested, total=total_configs,
                                progress=min(0.999, tested / float(max(1, total_configs))),
                                configs_per_second=float(tested) / elapsed_now,
                                best_profit=(0.0 if best_profit in (float("-inf"), float("inf")) else float(best_profit)),
                                count=1, gale=gale, gale_mode=mode
                            )
                    continue
                ...
                # V58.20: para esta combinação Gale/modo/contrária,
                # identifica quais quantidades/consensos já existem em SALVAS.
                skip_counts_np, skip_cons_np = _optimizer_excluded_pairs(
                    blocked_signatures,
                    gale,
                    mode,
                    contrary,
                ...
                # V58.20: a busca continua exata entre as configurações NÃO SALVAS.
                chunk = 30000
                for c0 in range(0, max_count, chunk):
                    ...
                    vals = _optimizer_sweep_chunk_numba(
                        ...
                        bool(financial_filter_enabled),
                        float(financial_target_value),
                        int(financial_target_mode_code),
                        int(signal_entry_every),
                        bool(min_entries_enabled),
                        int(min_entries),
                        bool(min_signal_pct_enabled),
                        float(min_signal_pct),
                        bool(dd_enabled),
                        float(dd_limit),
                        bool(cfg.get("white_protection_enabled", False)),
                        max(0.0, float(cfg.get("white_protection_stake", 1.0))),
                    ...
                    # V58.13: no objetivo PREJUÍZO vence o MENOR profit.
                    # Antes este comparador continuava usando ">" mesmo com
                    # best_profit iniciado em +inf; assim nunca aceitava o
                    # resultado do bloco e best_profit permanecia infinito.
                    chunk_better = (
                        float(chunk_profit) < best_profit
                        if objective == "loss"
                        else float(chunk_profit) > best_profit
                    )
                    if int(chunk_count) > 0 and (
                        ...
                        selected = ranked[:bc]
                        best_strategy = _optimizer_strategy_dict(
                            selected, gale, mode, bcon, cfg, contrary, signal_entry_every
                        )
                        dd_pct = (
                    ...
                    _optimizer_job_update(
                        ...
                        best_profit=(
                            0.0
                            if best_profit in (float("-inf"), float("inf"))
                            else float(best_profit)
                        ),
                        count=c1,
                        gale=gale,
                        gale_mode=mode
        ...
        if best_strategy is None:
            result_payload = {
                ...
                "history_count": n,
                "strategy_search_mode": str(strategy_search_mode),
                "engine": "V58.38 SINGLE/ENSEMBLE EXACT"
            }
        else:
            result_payload = {
                "ok": True,
                ...
                "rate_mode": rate_mode,
                "objective": objective,
                "financial_filter_enabled": bool(financial_filter_enabled),
                "financial_target_value": float(financial_target_value),
                "financial_target_mode": str(financial_target_mode),
                "signal_entry_every": int(signal_entry_every),
...
# ================================================================
# V58.19 — META-OTIMIZADOR ADAPTATIVO WALK-FORWARD
# Descobre automaticamente:
#   1) quantas rodadas usar para analisar o regime recente;
#   2) de quantas em quantas rodadas remodelar a estratégia.
# Não usa dados futuros para escolher uma estratégia no passado.
# Não altera histórico, PostgreSQL ou sessões existentes.
# ================================================================
META_JOBS = {}
META_JOBS_LOCK = threading.Lock()


def _meta_job_update(job_id, **changes):
    with META_JOBS_LOCK:
        job = META_JOBS.get(str(job_id))
        if isinstance(job, dict):
            job.update(changes)
            job["updated_at"] = agora_brasilia()


def _meta_prepare_prefix(colors_raw, max_history=10000):
    colors = list(colors_raw or [])[-max(100, int(max_history)):]
    n = len(colors)
    if n < 40:
        raise RuntimeError("Histórico insuficiente para META-otimização")

    occ = _optimizer_occurrences(colors)
    base = []
    for plen in range(1, 13):
        for mask in range(1 << plen):
            if len(base) >= 1500:
    ...
    # 1500 padrões x até 10k rodadas. uint16 é suficiente e mantém memória controlada.
    pe = np.zeros((len(base), n + 1), dtype=np.uint16)
    pr = np.zeros((len(base), n + 1), dtype=np.uint16)
    pb = np.zeros((len(base), n + 1), dtype=np.uint16)
    codes = np.fromiter(
        (1 if c == "R" else 2 if c == "B" else 0 for c in colors),
        dtype=np.uint8, count=n
    )

    for idx, item in enumerate(base):
        pos = np.asarray(item["positions"], dtype=np.int32)
        if pos.size == 0:
            continue
        row_e = np.zeros(n, dtype=np.uint8)
        row_r = np.zeros(n, dtype=np.uint8)
        row_b = np.zeros(n, dtype=np.uint8)
        row_e[pos] = 1
        valid_r = pos[codes[pos] == 1]
        valid_b = pos[codes[pos] == 2]
        if valid_r.size:
            row_r[valid_r] = 1
        if valid_b.size:
            row_b[valid_b] = 1
        pe[idx, 1:] = np.cumsum(row_e, dtype=np.uint16)
        pr[idx, 1:] = np.cumsum(row_r, dtype=np.uint16)
        pb[idx, 1:] = np.cumsum(row_b, dtype=np.uint16)

    return colors, base, pe, pr, pb


def _meta_window_arrays(pe, pr, pb, a, b):
    a = max(0, int(a)); b = max(a, int(b))
    entries = pe[:, b].astype(np.int32) - pe[:, a].astype(np.int32)
    wr = pr[:, b].astype(np.int32) - pr[:, a].astype(np.int32)
    wb = pb[:, b].astype(np.int32) - pb[:, a].astype(np.int32)
    return entries, wr, wb


def _meta_choose_from_window(base, pe, pr, pb, a, b, objective="profit", min_entries=2):
    entries, wr, wb = _meta_window_arrays(pe, pr, pb, a, b)
    min_entries = max(1, int(min_entries))
    valid = entries >= min_entries
    if not np.any(valid):
        valid = entries > 0
    if not np.any(valid):
        return None

    profit_r = 2.0 * wr - entries
    profit_b = 2.0 * wb - entries
    rate_r = np.where(entries > 0, 100.0 * wr / np.maximum(entries, 1), 0.0)
    rate_b = np.where(entries > 0, 100.0 * wb / np.maximum(entries, 1), 0.0)

    objective = str(objective or "profit").lower()
    invalid_low = -1.0e30

    def make_score(profit, rate):
        if objective == "loss":
            score = -profit.astype(np.float64)
        elif objective == "rate":
            # Taxa com pequeno desempate por quantidade de sinais.
            score = rate.astype(np.float64) + entries.astype(np.float64) * 1e-4
        elif objective == "signals":
            score = entries.astype(np.float64) + rate.astype(np.float64) * 1e-5
        else:
            # Lucro com pequeno desempate por frequência.
            score = profit.astype(np.float64) + entries.astype(np.float64) * 1e-6
        return np.where(valid, score, invalid_low)

    sr = make_score(profit_r, rate_r)
    sb = make_score(profit_b, rate_b)
    ir = int(np.argmax(sr)); ib = int(np.argmax(sb))
    choose_b = float(sb[ib]) > float(sr[ir])
    idx = ib if choose_b else ir
    target = "B" if choose_b else "R"
    wins = int(wb[idx] if choose_b else wr[idx])
    ent = int(entries[idx])
    profit = float((profit_b if choose_b else profit_r)[idx])
    rate = 100.0 * wins / ent if ent else 0.0
    item = base[idx]
    return {
        "type": "single",
        "name": "META • %s→%s" % ("".join(item["pattern"]), target),
        "pattern": list(item["pattern"]),
        "entry": target,
        "gale": 0,
        "gale_mode": 0,
        "meta_train_entries": ent,
        "meta_train_wins": wins,
        "meta_train_rate": rate,
        "meta_train_profit_units": profit
    }


def _meta_eval_strategy_block(strategy, base, pe, pr, pb, a, b):
    if not isinstance(strategy, dict):
        return {"entries": 0, "wins": 0, "losses": 0, "profit_units": 0.0}
    pattern = list(strategy.get("pattern", []))
    target = str(strategy.get("entry", ""))
    idx = -1
    plen = len(pattern)
    mask = 0
    for pos, c in enumerate(pattern):
        if c == "B":
            ...
            return {"entries": 0, "wins": 0, "losses": 0, "profit_units": 0.0}
    for i, item in enumerate(base):
        if int(item.get("plen", 0)) == plen and int(item.get("mask", -1)) == mask:
            idx = i
            break
    if idx < 0:
        return {"entries": 0, "wins": 0, "losses": 0, "profit_units": 0.0}

    e = int(pe[idx, b]) - int(pe[idx, a])
    w = (int(pr[idx, b]) - int(pr[idx, a])) if target == "R" else (int(pb[idx, b]) - int(pb[idx, a]))
    return {
        "entries": e,
        "wins": w,
        "losses": max(0, e - w),
        "profit_units": float(2 * w - e)
    }


def _meta_pair_score(result, objective):
    objective = str(objective or "profit").lower()
    if objective == "loss":
        return -float(result.get("profit_units", 0.0))
    if objective == "rate":
        return float(result.get("rate", 0.0))
    if objective == "signals":
        return float(result.get("entries", 0))
    return float(result.get("profit_units", 0.0))


def _meta_walk_forward(base, pe, pr, pb, n, analysis_window, interval, objective, min_train_entries):
    w = int(analysis_window); interval = int(interval)
    if w < 5 or interval < 1 or n <= w + interval:
        return None

    total_entries = total_wins = total_losses = 0
    total_profit = 0.0
    switches = 0
    blocks = 0
    last_sig = ""
    ...
    while t < n:
        train_a = max(0, t - w)
        chosen = _meta_choose_from_window(
            base, pe, pr, pb, train_a, t,
            objective=objective, min_entries=min_train_entries
        )
        if not chosen:
            t += interval
            continue
        end = min(n, t + interval)
        ev = _meta_eval_strategy_block(chosen, base, pe, pr, pb, t, end)
        sig = "%s>%s" % ("".join(chosen.get("pattern", [])), chosen.get("entry", ""))
        if last_sig and sig != last_sig:
            switches += 1
        last_sig = sig
        total_entries += int(ev["entries"])
        total_wins += int(ev["wins"])
        total_losses += int(ev["losses"])
        total_profit += float(ev["profit_units"])
        blocks += 1
        t = end

    if blocks <= 0:
        return None
    rate = 100.0 * total_wins / total_entries if total_entries else 0.0
    return {
        "analysis_window": w,
        "interval": interval,
        "blocks": blocks,
...
def _meta_recent_strategy(colors_raw, analysis_window, objective="profit", min_train_entries=2):
    colors, base, pe, pr, pb = _meta_prepare_prefix(colors_raw, max_history=max(1000, int(analysis_window) + 100))
    n = len(colors)
    a = max(0, n - int(analysis_window))
    return _meta_choose_from_window(base, pe, pr, pb, a, n, objective, min_train_entries)


def _meta_run_job(job_id, payload):
    started = time.time()
    try:
        colors_raw, history_ids = _optimizer_colors()
        if len(colors_raw) < 100:
            raise RuntimeError("Histórico insuficiente para META-otimizador")

        objective = str(payload.get("objective", "profit")).lower()
        if objective not in ("profit", "loss", "rate", "signals"):
            objective = "profit"
        min_train_entries = max(1, int(payload.get("min_train_entries", 2)))
        history_limit = max(500, min(10000, int(payload.get("history_limit", 7000))))
        intervals = payload.get("intervals", [3,5,7,10,15,20,30,50,100])
        windows = payload.get("analysis_windows", [20,50,100,200,500,1000])
        intervals = sorted(set(max(1, int(x)) for x in intervals if int(x) > 0))
        windows = sorted(set(max(10, int(x)) for x in windows if int(x) > 0))

        _meta_job_update(job_id, status="running", phase="PREPARANDO MATRIZ WALK-FORWARD", progress=0.02)
        colors, base, pe, pr, pb = _meta_prepare_prefix(colors_raw, max_history=history_limit)
        n = len(colors)
        windows = [w for w in windows if w < n - 5]
        if not windows:
            windows = [max(20, n // 4)]

        total_pairs = len(intervals) * len(windows)
        done = 0
        ranking = []
        best = None
        best_score = -1.0e300
        ...
        for w in windows:
            for interval in intervals:
                r = _meta_walk_forward(base, pe, pr, pb, n, w, interval, objective, min_train_entries)
                done += 1
                if r:
                    r["score"] = _meta_pair_score(r, objective)
                    ranking.append(r)
                ...
                _meta_job_update(
                    job_id, status="running",
                    phase="TESTANDO JANELA %d • REMODELAR %d" % (w, interval),
                    tested=done, total=total_pairs,
                    progress=min(0.97, done / float(max(1, total_pairs))),
                    best_interval=(int(best["interval"]) if best else 0),
                    best_window=(int(best["analysis_window"]) if best else 0),
                    best_profit_units=(float(best["profit_units"]) if best else 0.0)
                )

        if not best:
            raise RuntimeError("Nenhuma combinação META válida")

        ranking.sort(key=lambda x: float(x.get("score", -1e300)), reverse=True)
        current = _meta_choose_from_window(
            base, pe, pr, pb,
            max(0, n - int(best["analysis_window"])), n,
            objective=objective, min_entries=min_train_entries
        )
        if not current:
            raise RuntimeError("Não foi possível gerar a estratégia META atual")

        stake = float((payload.get("config", {}) or {}).get("stake", 10.0))
        meta_strategy = {
            "id": "META-%s-%s" % (int(time.time()), str(job_id)[:6]),
            "name": "META INTELIGENTE • %dr análise • remodela %dr" % (
                int(best["analysis_window"]), int(best["interval"])
            ),
            "type": "meta_adaptive",
            "meta_objective": objective,
            "meta_analysis_window": int(best["analysis_window"]),
            "meta_interval": int(best["interval"]),
            "meta_min_train_entries": int(min_train_entries),
            "meta_history_tested": int(n),
            "meta_walk_forward": dict(best),
            "meta_top_intervals": ranking[:10],
            "current_strategy": current,
            "gale": 0,
            "gale_mode": 0,
            "stake": stake,
            "initial_bank": float((payload.get("config", {}) or {}).get("initial_bank", 1000.0)),
        ...
        _meta_job_update(job_id, status="done", phase="META CONCLUÍDO", progress=1.0, result=result)
    except Exception as exc:
        _meta_job_update(job_id, status="error", phase="ERRO", error=str(exc), progress=1.0)


def meta_start(payload):
    job_id = hashlib.sha1(("meta" + str(time.time_ns()) + repr(payload)).encode()).hexdigest()[:16]
    with META_JOBS_LOCK:
        META_JOBS[job_id] = {
            "job_id": job_id, "status": "queued", "phase": "NA FILA",
            "progress": 0.0, "tested": 0, "total": 0,
            "created_at": agora_brasilia(), "updated_at": agora_brasilia()
        }
        if len(META_JOBS) > 10:
            for k in list(META_JOBS.keys())[:-10]:
                META_JOBS.pop(k, None)
    threading.Thread(target=_meta_run_job, args=(job_id, dict(payload or {})), daemon=True, name="meta-" + job_id).start()
    return {"ok": True, "job_id": job_id}


def meta_status(job_id):
    with META_JOBS_LOCK:
        job = META_JOBS.get(str(job_id))
        return dict(job) if isinstance(job, dict) else {"ok": False, "erro": "job META não encontrado"}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/meta/status"):
            try:
                ...
                for pair in parsed.query.split("&"):
                    if "=" in pair:
                        k, v = pair.split("=", 1)
                        qs[k] = v
                self.enviar_json(200, meta_status(qs.get("job_id", "")))
            except Exception as exc:
        ...
        if self.path.startswith("/persistencia"):
            st = postgres_status()
            with LOCK:
                total_mem = len(ESTADO.get("rodadas", []))
                demos_mem = len(ESTADO.get("demo_contas", {}))
            self.enviar_json(200, {
                "ok": True,
                "postgres_configurado": bool(st.get("configurado", False)),
                "postgres_online": bool(st.get("online", False)),
                "total_rodadas_postgres": int(st.get("total_rodadas_postgres", 0) or 0),
                "total_rodadas_memoria": int(total_mem),
                "demos_memoria": int(demos_mem),
                "erro_postgres": str(st.get("erro", "")),
                "regra_demo": "somente_rodadas_posteriores_a_ativacao",
                "historico_mutacao_no_boot": False
            })
            return
            ...
            if "?" in self.path:
                for parte in self.path.split("?", 1)[1].split("&"):
                    ...
                    if k == "demo_id":
                        ...
                        compact = str(v).lower() in ("1", "true", "sim", "yes")
            self.enviar_json(200, demo_status(demo_id, compact=compact))
            return
        ...
        if self.path.startswith("/reparo-lacunas-executar"):
            self.enviar_json(200, disparar_reparo_lacunas_background(origem="http_manual"))
            return

        if self.path == "/reparo-lacunas-status":
            with LOCK:
                status_reparo = dict(ESTADO.get("reparo_lacunas", {}) or {})
            diag = diagnosticar_continuidade_historico()
            diag.pop("segmento_atual", None)
            self.enviar_json(200, {
                "ok": True,
                "reparo": status_reparo,
                "continuidade": diag,
            })
            return
    ...
    def do_POST(self):
        ...
        if self.path == "/meta/start":
            try:
                obj = self.ler_json()
                self.enviar_json(200, meta_start(obj if isinstance(obj, dict) else {}))
            except Exception as exc:
        ...
        if self.path == "/demo/protecao-branco":
            try:
                obj = self.ler_json()
                demo_id = str(obj.get("demo_id", "principal")) if isinstance(obj, dict) else "principal"
                enabled = bool(obj.get("enabled", False)) if isinstance(obj, dict) else False
                stake = obj.get("stake", None) if isinstance(obj, dict) else None
                self.enviar_json(200, demo_protecao_branco(demo_id, enabled, stake))
            except Exception as exc:
                self.enviar_json(400, {"ok": False, "erro": str(exc)})
            return
        ...
        if self.path == "/demo/espelho":
            try:
                obj = self.ler_json()
                demo_id = str(obj.get("demo_id", "principal")) if isinstance(obj, dict) else "principal"
                enabled = bool(obj.get("enabled", False)) if isinstance(obj, dict) else False
                self.enviar_json(200, demo_espelho(demo_id, enabled))
            except Exception as exc:
                self.enviar_json(400, {"ok": False, "erro": str(exc)})
            return
...
def _optimizer_warmup_background():
    ...
    try:
        ...
        print(
            "V58.12 optimizer JIT pre-aquecido em %.3fs" % (time.time() - t0),
            flush=True
        )
    except Exception as exc:
...
def main():
    print("V58.32 ENTRADA A CADA N SINAIS + HISTORICO PRESERVADO: iniciando servidor...", flush=True)
    carregar_estado()
    with LOCK:
        _demos_boot = len(ESTADO.get("demo_contas", {}) or {})
    print("V58.17 demos restauradas no boot:", _demos_boot, flush=True)

    # V58.11: não executa corte/deleção da base durante deploy/restart.
    # Todo o restante do servidor, inclusive o otimizador V58.9, permanece intacto.
    print("V58.11 boot sem corte destrutivo do histórico", flush=True)
