...
# V58.36 — reparo por hora VALIDADA, aditivo e não destrutivo de lacunas históricas.
...
def _bestblaze_form_com_opcoes(html, base_url="https://bestblaze.com.br/doubleRodadas"):
    """Lê o mesmo formulário já usado pelo servidor e expõe as opções dos selects."""
    ...
    for _attrs, corpo in forms:
        # Usa o formulário que contém os campos de data reconhecidos.
...
def _bestblaze_escolher_campo_hora(form, selects):
    """Descobre o select de hora sem depender de um nome fixo do site."""
    candidatos = []
    for nome, opcoes in (selects or {}).items():
        baixo = str(nome).lower()
        score = 0
        if "hora" in baixo or "hour" in baixo:
            score += 100
        # Um seletor de hora normalmente possui muitas opções entre 0 e 23.
        horas = set()
        for opt in opcoes:
            for bruto in (opt.get("value", ""), opt.get("text", "")):
                m = re.search(r"(?<!\d)(?:[01]?\d|2[0-3])(?::00|h)?(?!\d)", str(bruto).lower())
                if m:
                    try:
                        horas.add(int(re.search(r"\d{1,2}", m.group(0)).group(0)))
                    except Exception:
                        pass
        if len(horas) >= 12:
            score += len(horas)
...
def _bestblaze_valores_opcao_hora(opcoes, hora):
    """V58.36 — candidatos de value para uma hora, priorizando o TEXTO visível.

    Alguns formulários usam value interno diferente da hora exibida (ex.: value=10
    para texto 09h). O V58.35 aceitava também o value como se fosse a própria hora,
    podendo selecionar a opção errada. Aqui o texto exibido ao usuário é a referência
    principal e o value só é fallback.
    """
    hora = int(hora) % 24
    exatos_texto = []
    fallback_valor = []
    ...
    formatos = {
        str(hora), f"{hora:02d}", f"{hora}h", f"{hora:02d}h",
        f"{hora}:00", f"{hora:02d}:00", f"{hora} h", f"{hora:02d} h"
    }

    for opt in opcoes or []:
        valor = str(opt.get("value", "")).strip()
        texto = re.sub(r"\s+", " ", str(opt.get("text", "")).strip().lower())
        if not valor:
            continue

        texto_norm = texto.replace("hs", "h").replace("hora", "h").strip()
        nums_texto = re.findall(r"\d{1,2}", texto_norm)
        n_texto = None
        if nums_texto:
            try:
                n_texto = int(nums_texto[0])
            except Exception:
                n_texto = None

        if n_texto == hora:
            score = 100
            if texto_norm in formatos:
                score = 200
            exatos_texto.append((score, valor))
            continue

        # Fallback: só usa o value se o texto não aponta claramente outra hora.
        nums_valor = re.findall(r"\d{1,2}", valor)
        if nums_valor:
            try:
                n_valor = int(nums_valor[0])
            except Exception:
                n_valor = None
            if n_valor == hora and n_texto is None:
                fallback_valor.append((20, valor))
                continue

        outros.append((0, valor))

    ordenados = []
    vistos = set()
    for grupo in (sorted(exatos_texto, reverse=True), sorted(fallback_valor, reverse=True), outros):
        for _score, valor in grupo:
...
def _bestblaze_valor_opcao_hora(opcoes, hora):
    """Compatibilidade: devolve o primeiro candidato validado pelo texto."""
    valores = _bestblaze_valores_opcao_hora(opcoes, hora)
    return valores[0] if valores else None


def _bestblaze_contar_hora_resposta(html, data_alvo, hora):
    """Conta timestamps da resposta que realmente pertencem ao dia/hora pedidos."""
    alvo_dia = data_alvo.strftime("%d/%m/%Y")
    hora = int(hora) % 24
    padrao = re.compile(r"(\d{2}/\d{2}/\d{4})\s+(\d{2}):(\d{2}):(\d{2})")
    total = 0
    fora = 0
    amostra = []
    for d, h, m, seg in padrao.findall(html_para_texto(html or "")):
        try:
            hh = int(h)
        except Exception:
            continue
        if d == alvo_dia and hh == hora:
            total += 1
            if len(amostra) < 3:
                amostra.append(f"{d} {h}:{m}:{seg}")
        else:
            fora += 1
    return total, fora, amostra


def buscar_hora_bestblaze(data_alvo, hora):
    """V58.36 — consulta uma hora e VALIDA a resposta antes de aceitá-la.

    O V58.35 confiava no value escolhido no select. Agora testamos candidatos de
    value e só aceitamos a resposta quando ela contém timestamps do dia/hora
    solicitados. Isso impede cruzar uma lacuna de 23h com resultados de outra hora.
    """
    base_url = "https://bestblaze.com.br/doubleRodadas"
    opener, jar = criar_sessao_bestblaze()
    ...
    if not form:
        raise RuntimeError("Formulário de período não identificado no HTML atual")

    campo_hora = _bestblaze_escolher_campo_hora(form, selects)
    if not campo_hora:
        raise RuntimeError("Filtro de hora da BestBlaze não pôde ser identificado com segurança")

    valores_hora = _bestblaze_valores_opcao_hora(selects.get(campo_hora, []), hora)
    if not valores_hora:
        raise RuntimeError("Nenhuma opção de hora candidata encontrada")
    ...
    inicio = data_alvo.replace(hour=int(hora), minute=0, second=0, microsecond=0)
    fim = inicio.replace(minute=59, second=59)
    tentativas = []
    melhor = None

    # Limita a poucas tentativas: normalmente a primeira (texto visível) já é correta.
    for valor_hora in valores_hora[:6]:
        payload = dict(form.get("defaults", {}))
        payload[form["campo_inicial"]] = formatar_data(inicio, form["tipo_inicial"])
        payload[form["campo_final"]] = formatar_data(fim, form["tipo_final"])
        payload[campo_hora] = str(valor_hora)
        payload = {str(k): "" if v is None else str(v) for k, v in payload.items() if str(k).strip()}

        encoded_text = urlencode(payload)
        encoded = encoded_text.encode("utf-8")
        if form["method"] == "POST":
            html, status_envio = abrir_com_sessao_bestblaze(opener, form["action"], data=encoded, referer=base_url)
        else:
            separador = "&" if "?" in form["action"] else "?"
            url_final = form["action"] + separador + encoded_text
            html, status_envio = abrir_com_sessao_bestblaze(opener, url_final, referer=base_url)

        no_alvo, fora, amostra = _bestblaze_contar_hora_resposta(html, data_alvo, hora)
        tentativa = {
            "valor_hora": str(valor_hora),
            "timestamps_no_alvo": int(no_alvo),
            "timestamps_fora": int(fora),
            "amostra": amostra,
            "status_http": int(status_envio),
        }
        tentativas.append(tentativa)

        if melhor is None or no_alvo > melhor[0]:
            melhor = (no_alvo, html, status_envio, valor_hora)
        if no_alvo > 0:
            # Resposta comprovadamente corresponde ao período pedido.
            return html, {
                "status_get": int(status_get),
                "status_envio": int(status_envio),
    ...
    # Não aceita silenciosamente uma resposta de outra hora.
    if melhor is not None and melhor[0] > 0:
        _, html, status_envio, valor_hora = melhor
        return html, {
            "status_get": int(status_get), "status_envio": int(status_envio),
            "campo_hora": campo_hora, "valor_hora": str(valor_hora),
            "data": data_alvo.strftime("%d/%m/%Y"), "hora": int(hora),
            "hora_validada": True, "timestamps_no_alvo": int(melhor[0]),
            "tentativas": tentativas,
        }

    raise RuntimeError(
        "BestBlaze respondeu, mas nenhuma tentativa trouxe timestamps do dia/hora solicitados: "
        + data_alvo.strftime("%d/%m/%Y") + f" {int(hora):02d}h"
    )


def _horas_necessarias_para_lacunas(lacunas, max_horas=240):
    """Lista somente as horas de relógio que interceptam alguma lacuna."""
    chaves = []
    vistos = set()
    for gap in lacunas:
        try:
            a = datetime.strptime(str(gap["de"]), "%d/%m/%Y %H:%M:%S")
            b = datetime.strptime(str(gap["ate"]), "%d/%m/%Y %H:%M:%S")
        except Exception:
            continue
        cursor = a.replace(minute=0, second=0, microsecond=0)
        fim = b.replace(minute=0, second=0, microsecond=0)
        while cursor <= fim and len(chaves) < max_horas:
            chave = cursor.strftime("%Y-%m-%d-%H")
...
def reparar_lacunas_reais_seguro(max_lacunas=500, max_dias=31):
    """
    V58.36 — reparo EXAUSTIVO com hora validada, porém estritamente aditivo.

    A diferença principal é que cada resposta por hora é validada pelo próprio timestamp da fonte. A página
    pública pode limitar a quantidade retornada). Agora consulta cada HORA que
    cruza uma lacuna, usando o filtro de hora real do formulário. Assim busca
    todas as rodadas disponíveis para os 25 buracos sem reconstruir a base.

    Nunca apaga, nunca substitui e nunca inventa rodada.
    """
    ...
    try:
        ...
        if not lacunas_todas:
            ...
            with LOCK:
                ...
                salvar_json(BANCO, ESTADO)
            return {"ok": True, **resumo, "mensagem": "Histórico já está contínuo."}

        # Repara as mais recentes primeiro, mas o padrão comporta todas as 25 atuais.
        lacunas = lacunas_todas[-max_lacunas:]
        intervalos = [
            (float(g["de_epoch"]), float(g["ate_epoch"]))
            for g in lacunas
            if float(g.get("de_epoch", 0)) > 0 and float(g.get("ate_epoch", 0)) > float(g.get("de_epoch", 0))
        ]

        # Limita por número de dias distintos pedido, sem truncar horas do mesmo dia.
        dias_permitidos = []
        for g in reversed(lacunas):
            for campo in ("ate", "de"):
                try:
                    dia = datetime.strptime(g[campo], "%d/%m/%Y %H:%M:%S").strftime("%Y-%m-%d")
                except Exception:
                    continue
                if dia not in dias_permitidos:
                    dias_permitidos.append(dia)
                if len(dias_permitidos) >= max_dias:
                    break
            if len(dias_permitidos) >= max_dias:
                break
        permitidos = set(dias_permitidos)

        horas = [h for h in _horas_necessarias_para_lacunas(lacunas, max_horas=744) if h.strftime("%Y-%m-%d") in permitidos]
        candidatos = []
        consultas = []
        fallback_dias = set()

        for idx, alvo in enumerate(horas, 1):
            with LOCK:
                ESTADO["reparo_lacunas"] = {
                    "status": "executando",
                    "etapa": "consultando_horas",
                    "progresso": f"{idx}/{len(horas)}",
                    "hora_atual": alvo.strftime("%d/%m/%Y %Hh"),
                    "lacunas_antes": int(antes.get("lacunas", 0)),
                    "adicionadas": 0,
                    "erro": "",
                }
            try:
                html, diag = buscar_hora_bestblaze(alvo, alvo.hour)
                ...
                for rodada in reconhecidas:
                    ...
                    if t > 0 and any(a < t < b for a, b in intervalos):
                        candidatos.append(dict(rodada))
                        aceitas += 1
                consultas.append({
                    "periodo": alvo.strftime("%d/%m/%Y %Hh"),
                    "modo": "hora",
                    "ok": True,
                    "reconhecidas_fonte": len(reconhecidas),
                    "candidatas_em_lacunas": aceitas,
                    "status_http": int(diag.get("status_envio", 0)),
                })
            except Exception as exc:
                # Fallback conservador: um dia inteiro, uma única vez por dia.
                fallback_dias.add(alvo.strftime("%Y-%m-%d"))
                consultas.append({"periodo": alvo.strftime("%d/%m/%Y %Hh"), "modo": "hora", "ok": False, "erro": str(exc)})

        # Se o site mudou o select de hora, ainda tentamos o método V58.34 por dia.
        for dia_txt in sorted(fallback_dias):
            alvo = datetime.strptime(dia_txt, "%Y-%m-%d")
            ...
            try:
                ...
                for rodada in reconhecidas:
                    ...
                    if t > 0 and any(a < t < b for a, b in intervalos):
                        ...
                        aceitas += 1
                consultas.append({
                    "periodo": alvo.strftime("%d/%m/%Y"), "modo": "dia_fallback", "ok": True,
                    "reconhecidas_fonte": len(reconhecidas), "candidatas_em_lacunas": aceitas,
                    "status_http": int(diag.get("status_envio", 0)),
                })
            except Exception as exc:
                consultas.append({"periodo": alvo.strftime("%d/%m/%Y"), "modo": "dia_fallback", "ok": False, "erro": str(exc)})

        unicos = []
        chaves = set()
        for rodada in candidatos:
            # Horário é a chave mais segura aqui: uma única rodada por instante.
            chave = str(rodada.get("data_hora", ""))
            if not chave or chave in chaves:
                continue
            chaves.add(chave)
            unicos.append(rodada)
        ...
        with LOCK:
            ESTADO["reparo_lacunas"] = {
                "status": "erro_sem_alteracao_destrutiva",
                "ultima_execucao": agora_brasilia(),
                "adicionadas": 0,
                "erro": str(exc),
            }
...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        cache_payload = {
            "engine": "v58.30-history-window-exact",
            "history": hist_hash,
