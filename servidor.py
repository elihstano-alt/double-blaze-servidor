...
@njit(cache=True)
def _optimizer_numba_core_sparse(
    colors,
    positions,
    target_codes,
    gale,
    ...
    rate = 100.0 * wins / entries if entries else 0.0
    return (
        entries, wins, losses, gale_wins, bank - initial_bank, rate, bank,
        max_dd, max_gale_reached, largest_stake, max_cycle_exposure,
        min_bank, bankrupt_stops, peak, bank_at_max_dd, peak_at_max_dd
    )
...
@njit(cache=True)
def _optimizer_sweep_chunk_sparse_numba(
    colors,
    flat_positions,
    position_offsets,
    entry_codes,
    ...
    """
    V56.9 - motor esparso:
    - não cria matriz 10000 x histórico;
    - cada nova estratégia atualiza somente as posições em que realmente sinaliza;
    ...
    target_codes = np.empty(n, dtype=np.uint8)

    for count0 in range(start_count, end_count):
        p0 = int(position_offsets[count0])
        p1 = int(position_offsets[count0 + 1])
        entry = int(entry_codes[count0])

        # Atualiza somente as ocorrências da estratégia recém-adicionada.
        if entry == 1:
            for k in range(p0, p1):
                votes_r[int(flat_positions[k])] += 1
        else:
            for k in range(p0, p1):
                votes_b[int(flat_positions[k])] += 1
        ...
        for ci in range(consensuses.shape[0]):
            ...
            # Um único passe nativo pelo histórico para materializar os sinais
            # válidos deste consenso; o simulador recebe somente arrays esparsos.
            for i in range(n):
                vr = int(votes_r[i])
                vb = int(votes_b[i])
                active = vr + vb
            ...
            vals = _optimizer_numba_core_sparse(
                colors,
                positions[:m],
                target_codes[:m],
                gale,
...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        max_count_req = max(1, min(10000, int(payload.get("max_count", 10000))))
        ...
        cache_payload = {
            "engine": "v56.9-sparse-jit-10k-exact",
            "history": hist_hash,
        ...
        if isinstance(cached, dict):
            ...
            _optimizer_job_update(
                ...
                phase="CACHE COMPLETO V56.9", result=cached
            )
            return
        ...
        _optimizer_job_update(
            ...
            phase="COMPILANDO MOTOR SPARSE JIT V56.9 (SÓ NA 1ª VEZ)",
        ...
        _fp = np.asarray([1], dtype=np.int32)
        _offs = np.asarray([0, 1], dtype=np.int64)
        _ec = np.asarray([1], dtype=np.uint8)
        _vr = np.zeros(6, dtype=np.uint16)
        _vb = np.zeros(6, dtype=np.uint16)
        _optimizer_sweep_chunk_sparse_numba(
            _wc, _fp, _offs, _ec, _vr, _vb,
        ...
        _optimizer_job_update(
            job_id, status="running",
            phase="MOTOR SPARSE JIT V56.9 PRONTO",
            tested=0, total=total_configs, progress=0.0
        )
        ...
        for gale in gales:
            for mode in modes:
                _optimizer_job_update(
                    job_id,
                    phase="RANKING EXATO 16380 • G%d • M%d" % (gale, mode),
                    tested=tested, total=total_configs,
                    progress=min(0.999, tested / float(max(1, total_configs)))
                )

                # Ranking individual exato das 16380 estratégias.
                ranked = []
                for idx, cand0 in enumerate(base):
                    ...
                    if idx % 128 == 0:
                        _optimizer_job_update(
                            job_id,
                            phase="RANKING EXATO 16380 • %d/16380" % min(16380, idx * 2),
                            tested=tested, total=total_configs
                        )
                ...
                # V56.9: representação ESPARSA.
                # Em vez de 10.000 x N bytes, armazena apenas as posições onde
                # cada estratégia realmente gera sinal.
                entry_codes = np.zeros(max_count, dtype=np.uint8)
                position_offsets = np.zeros(max_count + 1, dtype=np.int64)
                flat_parts = []
                flat_total = 0
                for ri, cand in enumerate(ranked):
                    pos = np.asarray(cand["positions"], dtype=np.int32)
                    flat_parts.append(pos)
                    flat_total += int(pos.size)
                    position_offsets[ri + 1] = flat_total
                    entry_codes[ri] = 1 if cand["entry"] == "R" else 2

                if flat_total > 0:
                    flat_positions = np.concatenate(flat_parts).astype(np.int32, copy=False)
                else:
                    flat_positions = np.empty(0, dtype=np.int32)

                votes_r = np.zeros(n, dtype=np.uint16)
                votes_b = np.zeros(n, dtype=np.uint16)

                # Bloco maior reduz overhead Python sem cortar configurações.
                chunk = 250
                for c0 in range(0, max_count, chunk):
                    c1 = min(max_count, c0 + chunk)
                    vals = _optimizer_sweep_chunk_sparse_numba(
                        colors,
                        flat_positions,
                        position_offsets,
                        entry_codes,
        ...
        print("Optimizer remoto V56.9 erro:", repr(exc), flush=True)
...
def main():
    print("V56.6 HISTORICO RECUPERADO + NUMBA: janela 30000 + sem I/O por configuração: iniciando servidor...", flush=True)
    carregar_estado()
