...
def _optimizer_excluded_pairs(blocked_signatures, gale, mode, contrary,
    ...
    for raw in blocked_signatures or []:
        ...
        m_single = re.match(
            r"^single\|.*\|g(\d+)\|m(\d+)\|inv(true|false)"
        ...
        m_ens = re.match(
            r"^ensemble\|(.*)\|g(\d+)\|m(\d+)\|c(\d+)\|inv(true|false)"
            r"(?:\|wp(true|false)\|w([0-9.]+))?$",
            sig, flags=re.IGNORECASE
        )
        if not m_ens:
...
def main():
    print("V58.24 PROTECAO BRANCO CORRIGIDA + ESPELHO A/B + PERSISTENCIA: iniciando servidor...", flush=True)
    carregar_estado()
    with LOCK:
