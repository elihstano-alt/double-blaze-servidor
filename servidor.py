...
def _demo_processar_espelho(session, rodada, previous_colors):
    ...
    if can_a:
        session["mirror_wins_a"] = int(session.get("mirror_wins_a", 0)) + (1 if win_a else 0)
        session["mirror_losses_a"] = int(session.get("mirror_losses_a", 0)) + (0 if win_a else 1)
    if can_b:
        session["mirror_wins_b"] = int(session.get("mirror_wins_b", 0)) + (1 if win_b else 0)
        session["mirror_losses_b"] = int(session.get("mirror_losses_b", 0)) + (0 if win_b else 1)
    if actual == "W":
    ...
    ops.insert(0, {
        ...
        "b_alvo": target_b,
        "resultado": actual,
        "a_resultado": "WIN" if win_a else "LOSS",
        "b_resultado": "WIN" if win_b else "LOSS",
        "stake": round(stake, 2),
