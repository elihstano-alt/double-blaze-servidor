...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        _optimizer_job_update(
            ...
            phase="COMPILANDO SWEEP JIT V56.5 (SÓ NA 1ª VEZ)",
        ...
        for gale in gales:
            for mode in modes:
                _optimizer_job_update(
                    job_id,
                    phase="RANKING EXATO 2044 • G%d • M%d" % (gale, mode),
                    tested=tested, total=total_configs,
                    progress=min(0.999, tested / float(max(1, total_configs)))
                )

                # Ranking individual exato das 2044 estratégias.
                ranked = []
                for idx, cand0 in enumerate(base):
                    ...
                    if idx % 128 == 0:
                        _optimizer_job_update(
                            job_id,
                            phase="RANKING EXATO 2044 • %d/2044" % min(2044, idx * 2),
                            tested=tested, total=total_configs
                        )
                ...
                # 1000 x 30000 = ~30 MB e evita recriar sinais milhões de vezes.
        ...
        print("Optimizer remoto V56.5 erro:", repr(exc), flush=True)
