...
# V58.34 — reparo auditável, aditivo e não destrutivo de lacunas históricas.
...
def reparar_lacunas_reais_seguro(max_lacunas=60, max_dias=7):
    """
    Preenche SOMENTE lacunas comprovadas com rodadas reais retornadas pela
    fonte pública já usada pelo servidor. Operação estritamente aditiva:

      * não remove nem substitui rodadas existentes;
      * não altera ID/cor/horário de registro existente;
      * só aceita R/B/W parseado da fonte;
      * só insere candidato cujo horário esteja estritamente dentro de uma
        lacuna que existia antes da consulta;
      * adicionar_rodadas_em_lote() mantém deduplicação por id/horário e
        persiste apenas as novas rodadas no PostgreSQL;
      * importação histórica não alimenta a Demo.

    Se a fonte não devolver uma rodada válida, nada é inventado.
    """
    ...
    try:
        ...
        if not lacunas_todas:
            ...
            return {
                ...
                "lacunas_antes": 0,
                "lacunas_depois": 0,
                "mensagem": "Histórico já está contínuo.",
            }
        ...
        dias = []
        vistos = set()
        for gap in reversed(lacunas):
            for campo in ("ate", "de"):
                try:
                    dt = datetime.strptime(gap[campo], "%d/%m/%Y %H:%M:%S")
        ...
        intervalos = [
            ...
            for g in lacunas
            if float(g.get("de_epoch", 0)) > 0
            and float(g.get("ate_epoch", 0)) > float(g.get("de_epoch", 0))
        ]
        ...
        for alvo in dias:
            ...
            try:
                ...
                for rodada in reconhecidas:
                    ...
                    if t <= 0:
                        continue
                    if any(a < t < b for a, b in intervalos):
                ...
                dias_consultados.append({
                    ...
                    "candidatas_em_lacunas": aceitas_dia,
                    "status_http": int(diag.get("status_envio", 0)),
                })
            except Exception as exc:
                dias_consultados.append({
        ...
        resultado_lote = adicionar_rodadas_em_lote(unicos) if unicos else {
            "recebidas": 0, "adicionadas": 0, "duplicadas": 0
        }
        depois = diagnosticar_continuidade_historico()
...
def disparar_reparo_lacunas_background(origem="manual"):
    ...
    def _run():
        ...
        resultado = reparar_lacunas_reais_seguro(
            max_lacunas=max(1, int(os.getenv("REPARO_LACUNAS_MAX", "60"))),
            max_dias=max(1, int(os.getenv("REPARO_LACUNAS_MAX_DIAS", "7"))),
        )
...
def worker_reparo_inicial():
    """
    V58.34 — reparo seguro, auditável, aditivo e não destrutivo.
