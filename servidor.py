...
def _normalizar_url_bestblaze(url):
    """
    Mantém o host canônico sem www.
    O domínio www.bestblaze.com.br está apresentando certificado TLS
    incompatível; não desativamos a validação SSL.
    """
    valor = str(url or "").strip()
    valor = re.sub(
        r"(?i)^https://www\\.bestblaze\\.com\\.br(?=/|$)",
        "https://bestblaze.com.br",
        valor
    )
    return valor
...
class _BestBlazeRedirectHandler(HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        newurl = _normalizar_url_bestblaze(newurl)
...
def buscar_html_publico(url):
    ...
    opener = build_opener(_BestBlazeRedirectHandler())
...
def _demo_processar_sessao(session, rodada, previous_colors):
    ...
    if is_white:
        max_attempts = max(1, min(20, int(session.get("white_attempts", strategy.get("attempts", 6)))))
        gale_max = max_attempts - 1
        gale_mode = 0
    else:
        gale_max = max(0, min(11, int(strategy.get("gale", 0))))
        gale_mode = int(strategy.get("gale_mode", 0))
    white_enabled = bool(session.get("white_enabled", True))
    ...
    skip_remaining = int(session.get("skip_remaining", 0))
...
def _demo_id_valido(demo_id):
    return str(demo_id) in {f"demo{i}" for i in range(1, 8)} or str(demo_id) == "principal"
...
def demo_iniciar(payload):
    ...
    if not _demo_id_valido(demo_id):
        raise ValueError("demo_id inválido; use demo1 até demo7")
    ...
    # No branco, o número de tentativas pertence à estratégia salva.
    # A conta Demo não pode sobrescrever esse valor.
    white_attempts = max(1, min(20, int(strategy.get("attempts", 6))))
    if str(strategy.get("type", "single")) == "white":
        gale_max = white_attempts - 1
    else:
        gale_max = max(0, min(11, int(strategy.get("gale", 0))))
    ...
    if str(sessao["strategy"].get("type", "single")) == "white":
        sessao["strategy"]["attempts"] = white_attempts
...
def demo_alterar_estrategia(payload):
    ...
    with LOCK:
        ...
        white_attempts = max(1, min(20, int(strategy.get("attempts", 6))))
        sessao["white_attempts"] = white_attempts
        if str(sessao["strategy"].get("type", "single")) == "white":
            sessao["strategy"]["attempts"] = white_attempts
...
def demo_tentativas_branco(demo_id="principal", attempts=6):
    # Compatibilidade com clientes antigos: não altera mais o número de tentativas.
    # Esse valor agora é definido exclusivamente quando a estratégia de branco é gerada/salva.
    with LOCK:
        sessao = dict(ESTADO.get("demo_contas", {}).get(str(demo_id), {}) or {})
        if not sessao:
            raise ValueError("demo não encontrada")
        strategy = dict(sessao.get("strategy", {}) or {})
        if str(strategy.get("type", "single")) == "white":
            sessao["white_attempts"] = max(1, min(20, int(strategy.get("attempts", 6))))
            ESTADO.setdefault("demo_contas", {})[str(demo_id)] = sessao
            salvar_json(BANCO, ESTADO)
    if postgres_configurado():
        postgres_demo_salvar(demo_id, sessao)
    out = demo_status(demo_id)
    out["tentativas_controladas_pela_estrategia"] = True
    return out
...
def detectar_formulario_periodo_bestblaze(html, base_url):
    ...
    for attrs, corpo in forms:
        ...
        return {
            "method": metodo,
            "action": _normalizar_url_bestblaze(urljoin(base_url, action)),
...
def main():
    print("V54.8 Bootstrap único persistente + Auditoria contínua + janela 5000: iniciando servidor...", flush=True)
    carregar_estado()
