...
# V58.35 — reparo exaustivo por hora, aditivo e não destrutivo de lacunas históricas.
...
def _bestblaze_valor_opcao_hora(opcoes, hora):
    """Retorna exatamente o value que o site espera para uma hora (0..23)."""
    hora = int(hora) % 24
    melhores = []
    for opt in opcoes or []:
        valor = str(opt.get("value", ""))
        texto = str(opt.get("text", ""))
        score = -1
        for bruto in (valor, texto):
            s = bruto.strip().lower()
            ...
            if n != hora:
                continue
            score = max(score, 10)
            if s in (str(hora), f"{hora:02d}", f"{hora}h", f"{hora:02d}h", f"{hora}:00", f"{hora:02d}:00"):
                score = max(score, 30)
        if score >= 0:
            melhores.append((score, valor))
    if not melhores:
        return None
...
def buscar_hora_bestblaze(data_alvo, hora):
    """
    Consulta UMA hora específica do histórico público. Isso evita o limite de
    quantidade da página quando um dia inteiro é solicitado.
    """
    base_url = "https://bestblaze.com.br/doubleRodadas"
    opener, jar = criar_sessao_bestblaze()
    ...
    valor_hora = _bestblaze_valor_opcao_hora(selects.get(campo_hora, []), hora) if campo_hora else None
    if not campo_hora or valor_hora is None:
...
def reparar_lacunas_reais_seguro(max_lacunas=500, max_dias=31):
    """
    V58.35 — reparo EXAUSTIVO, porém estritamente aditivo.

    A diferença para V58.34 é que não consulta apenas o dia inteiro (a página
