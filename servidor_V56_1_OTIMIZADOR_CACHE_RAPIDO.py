...
def _optimizer_run_job(job_id, payload):
    started = time.time()
    try:
        colors, history_ids = _optimizer_colors()
        n = len(colors)
        if n < 20:
        ...
        cache_key = hashlib.sha256(json.dumps(cache_payload, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
        with OPTIMIZER_CACHE_LOCK:
            cached = OPTIMIZER_RESULT_CACHE.get(cache_key)
        if isinstance(cached, dict):
            cached = dict(cached)
            cached["cached"] = True
            _optimizer_job_update(job_id, status="done", progress=1.0, phase="CACHE PRONTO", result=cached)
            return

        total_configs = len(gales) * len(modes) * max_count_req * len(consensuses)
        _optimizer_job_update(job_id, status="running", phase="PREPARANDO CACHE V56.1", tested=0, total=total_configs, progress=0.0)

        # 1) Ocorrências dos 1022 padrões são calculadas UMA vez.
        ...
        for plen in range(1, 10):
            for mask in range(1 << plen):
                ...
                base.append({"plen": plen, "mask": mask, "pattern": pattern, "positions": positions})
        ...
        # V56.1: não cria bytearray de 30k para cada uma das 2044 estratégias.
        # Para uma estratégia individual, positions já É a máscara esparsa.
        # Só materializamos as máscaras acumuladas do ensemble uma vez por (gale,modo).
        for gale in gales:
            for mode in modes:
                _optimizer_job_update(
                    job_id, phase="RANKING RÁPIDO 2044", gale=gale, gale_mode=mode,
                    tested=tested, total=total_configs
                )
                ranked = []

                # Ranking individual: reutiliza a mesma lista de posições para R e B.
                for idx, cand0 in enumerate(base):
                    positions = cand0["positions"]
                    for entry, target_code in (("R", 1), ("B", 2)):
                        signal_mask = bytearray(n)
                        targets = bytearray(n)
                        for pos in positions:
                            signal_mask[pos] = 1
                            targets[pos] = target_code
                        r = _optimizer_simulate_mask(colors, signal_mask, targets, cfg, gale, mode)
                    ...
                    if idx % 64 == 0:
                        _optimizer_job_update(job_id, phase="RANKING RÁPIDO 2044", phase_current=idx*2, phase_total=2044)
                ...
                # Arrays compactos; cada nova estratégia altera somente suas posições.
                votes_r = [0] * n
                votes_b = [0] * n
                targets_by_consensus = {c: bytearray(n) for c in consensuses}
                masks_by_consensus = {c: bytearray(n) for c in consensuses}

                for count in range(1, max_count + 1):
                    cand = ranked[count - 1]
                    target_vote = 1 if cand["entry"] == "R" else 2

                    # Atualização incremental: recalcula consenso SOMENTE nas posições
                    # afetadas pela estratégia recém-adicionada.
                    for pos in cand["positions"]:
                        if target_vote == 1:
                            votes_r[pos] += 1
                        else:
                            votes_b[pos] += 1
                        vr, vb = votes_r[pos], votes_b[pos]
                        active = vr + vb
                        if active <= 0 or vr == vb:
                            for con in consensuses:
                                targets_by_consensus[con][pos] = 0
                                masks_by_consensus[con][pos] = 0
                            continue
                        winner = vr if vr > vb else vb
                        natural = 1 if vr > vb else 2
                        pct = 100.0 * winner / active
                        for con in consensuses:
                            required = 50.0001 if con <= 50 else float(con)
                            t = natural if pct + 0.0001 >= required else 0
                            if contrary and t:
                                t = 2 if t == 1 else 1
                            targets_by_consensus[con][pos] = t
                            masks_by_consensus[con][pos] = 1 if t else 0

                    selected = ranked[:count]
                    for con in consensuses:
                        r = _optimizer_simulate_mask(
                            colors, masks_by_consensus[con], targets_by_consensus[con],
                            cfg, gale, mode
                        )
                        tested += 1
                        rate = float(r.get("rate", 0.0))
                        ...
                        if dd_enabled and dd > dd_limit:
                            passes = False
                        if passes and (profit > best_profit or (abs(profit - best_profit) < 1e-9 and dd < best_dd)):
                            best_profit = profit
                            best_dd = dd
                            best_result = r
                            best_strategy = _optimizer_strategy_dict(selected, gale, mode, con, cfg, contrary)

                        if tested % 100 == 0:
                            _optimizer_job_update(
                                job_id, phase="VARREDURA EXATA V56.1", tested=tested, total=total_configs,
        ...
        if best_strategy is None:
            result_payload = {
                "ok": False, "message": "Nenhuma configuração passou pelos filtros.",
                "tested": tested, "total": total_configs,
        ...
        _optimizer_job_update(
            job_id, status="done", phase="CONCLUÍDO V56.1", progress=1.0,
            tested=tested, total=total_configs, result=result_payload
        )
    except Exception as exc:
        _optimizer_job_update(job_id, status="error", phase="ERRO", error=str(exc), progress=1.0)
        print("Optimizer remoto V56.1 erro:", repr(exc))
...
def main():
    print("V56.1 OTIMIZADOR CACHE RÁPIDO + BRANCO MANUAL: janela 30000 + busca exata + cache: iniciando servidor...", flush=True)
    carregar_estado()
