...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        cache_payload = {
            "engine": "v58.2-rate-min-max-nonzero-exact",
            "history": hist_hash,
...
def _optimizer_warmup_background():
    ...
    try:
        ...
        print(
            "V58.2 optimizer JIT pre-aquecido em %.3fs" % (time.time() - t0),
            flush=True
        )
    except Exception as exc:
        print("V58.2 warmup optimizer falhou:", repr(exc), flush=True)


def main():
    print("V58.2 ACERTO MIN/MAX SEM ZERO ENTRADAS + BASE AUDITADA + COLETA REDUNDANTE: iniciando servidor...", flush=True)
    carregar_estado()
