...
def demo_recuperar_pendentes():
    ...
    for demo_id, sessao in demos.items():
        ...
        start = _demo_inicio_pendente(base, sessao)
        atual = dict(sessao)
        antes = int(atual.get("processed_rounds", 0))
        ...
        depois = int(atual.get("processed_rounds", 0))
        processadas = max(0, depois - antes)
        total_processadas += processadas
        if processadas > 0:
            demos_atualizadas += 1

        atual["sync_last_check"] = agora_brasilia()
        atual["sync_recovered_rounds"] = processadas
        atual["sync_server_latest_id"] = str(base[-1].get("id", ""))
        atual["sync_server_latest_time"] = str(base[-1].get("data_hora", ""))
    ...
    return {
        "demos": demos_atualizadas,
        "processadas": total_processadas,
        "ultima_rodada_servidor": str(base[-1].get("data_hora", ""))
    }


def worker_demo_sincronia_24h():
    """
    Rede de segurança independente da chegada ao vivo.
    A cada 10s, compara cada Demo ativa com o histórico já salvo e processa
    tudo que estiver pendente. Funciona mesmo com o app fechado.

    Se o WebSocket cair e a reconciliação histórica recuperar rodadas em lote,
    a Demo também as processará em ordem cronológica.
    """
    time.sleep(8)
    while True:
        try:
            info = demo_recuperar_pendentes()
            if int(info.get("processadas", 0)) > 0:
                print(
                    "V58.4 Demo catch-up:",
                    info.get("processadas", 0),
                    "rodada(s) recuperada(s)",
                    flush=True
                )
        except Exception as exc:
            print("V58.4 Demo sync erro:", repr(exc), flush=True)
        time.sleep(10)
...
def demo_status(demo_id="principal"):
    ...
    start_pending = _demo_inicio_pendente(base, sessao) if base else 0
    pending = max(0, len(base) - start_pending) if bool(sessao.get("active", False)) else 0
...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        cache_payload = {
            "engine": "v58.3-rate-max-lowrate-ranking-exact",
            "history": hist_hash,
...
def _optimizer_warmup_background():
    ...
    try:
        ...
        print(
            "V58.4 optimizer JIT pre-aquecido em %.3fs" % (time.time() - t0),
            flush=True
        )
    except Exception as exc:
        print("V58.4 warmup optimizer falhou:", repr(exc), flush=True)


def main():
    print("V58.4 DEMO 24H SINCRONIA + CONTRARIO AUDITADO + ACERTO MAXIMO: iniciando servidor...", flush=True)
    carregar_estado()

    try:
        corte_info = aplicar_corte_base_auditada_uma_vez()
        print("V58.0 corte base auditada:", corte_info, flush=True)
    except Exception as exc:
        print("V58.0 ERRO no corte auditado:", exc, flush=True)
        # Não derruba o servidor; preserva os dados caso o banco esteja indisponível.
