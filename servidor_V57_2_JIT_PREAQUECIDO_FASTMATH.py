...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        _optimizer_job_update(
            ...
            phase="PREPARANDO MOTOR JIT V57.2",
        ...
        if best_strategy is None:
            result_payload = {
                ...
                "history_count": n,
                "engine": "V57.2 DENSE JIT 10K EXACT FASTMATH"
            }
        else:
            result_payload = {
                "ok": True,
...
def _optimizer_warmup_background():
    """V57.2: compila os kernels Numba logo após o boot, fora do primeiro clique do usuário."""
    try:
        t0 = time.time()
        _wc = np.asarray([1, 2, 1, 2, 1, 2], dtype=np.uint8)
        _sm = np.zeros((1, 6), dtype=np.uint8)
        ...
        print(
            "V57.2 optimizer JIT pre-aquecido em %.3fs" % (time.time() - t0),
            flush=True
        )
    except Exception as exc:
        print("V57.2 warmup optimizer falhou:", repr(exc), flush=True)
...
def main():
    ...
    # V57.2: o custo de compilação JIT acontece em background no boot,
