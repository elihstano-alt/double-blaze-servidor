...
def _demo_processar_sessao(session, rodada, previous_colors):
    ...
    if not cycle_active:
        if signal:
            # V57.5: SINAL CONTRÁRIO afeta somente NOVOS sinais R/B.
            # Gale/ciclo já em andamento mantém o alvo original.
            if bool(session.get("contrary_signal", False)) and signal in ("R", "B"):
                signal = "B" if signal == "R" else "R"
            cycle_active = True
            current_gale = 0
            cycle_target = signal
            should_bet = True
...
def demo_sinal_contrario(demo_id="principal", enabled=True):
    with LOCK:
        ...
        if not sessao:
            raise ValueError("demo não encontrada")
        # Não reinicia nem altera Gale/ciclo atual.
        # A mudança vale para o próximo novo sinal.
...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        cache_payload = {
            "engine": "v57.5-dense-jit-3k-ranking-cache-exact",
            "history": hist_hash,
        ...
        _optimizer_job_update(
            ...
            phase="PREPARANDO MOTOR JIT V57.5",
        ...
        for gale in gales:
            for mode in modes:
                ...
                # V57.5: bloco maior apenas para reduzir overhead.
        ...
        if best_strategy is None:
            result_payload = {
                ...
                "history_count": n,
                "engine": "V57.5 DENSE JIT 10K EXACT FASTMATH"
            }
        else:
            result_payload = {
                "ok": True,
...
def _optimizer_warmup_background():
    """V57.5: compila os kernels Numba logo após o boot, fora do primeiro clique do usuário."""
    try:
        t0 = time.time()
        _wc = np.asarray([1, 2, 1, 2, 1, 2], dtype=np.uint8)
        _sm = np.zeros((1, 6), dtype=np.uint8)
        ...
        print(
            "V57.5 optimizer JIT pre-aquecido em %.3fs" % (time.time() - t0),
            flush=True
        )
    except Exception as exc:
        print("V57.5 warmup optimizer falhou:", repr(exc), flush=True)
...
def main():
    ...
    # V57.5: o custo de compilação JIT acontece em background no boot,
