...
def _optimizer_run_job(job_id, payload):
    ...
    try:
        ...
        _optimizer_job_update(job_id, status="running", phase="PREPARANDO PADRÕES", tested=0, total=total_configs, progress=0.0)
        ...
        for plen in range(1, 10):
            for mask in range(1 << plen):
                ...
                for entry in ("R", "B"):
                    base.append({"plen": plen, "mask": mask, "pattern": pattern, "entry": entry, "positions": positions})
        ...
        # Cache de ranking por (gale,mode), pois é a parte individual cara.
        ranking_cache = {}
        for gale in gales:
            for mode in modes:
                _optimizer_job_update(job_id, phase="RANQUEANDO 2044 ESTRATÉGIAS", gale=gale, gale_mode=mode, tested=tested, total=total_configs)
                rank_key = (gale, mode)
                ranked = []
                for idx, cand in enumerate(base):
                    target_code = 1 if cand["entry"] == "R" else 2
                    mask_bytes, target_bytes = _optimizer_single_mask(n, cand["positions"], target_code)
                    r = _optimizer_simulate_mask(colors, mask_bytes, target_bytes, cfg, gale, mode)
                    if int(r.get("entries", 0)) >= 5:
                        item = dict(cand)
                        item["profit"] = float(r.get("profit", float("-inf")))
                        item["rate"] = float(r.get("rate", 0.0))
                        ranked.append(item)
                    if idx % 128 == 0:
                        _optimizer_job_update(job_id, phase="RANQUEANDO 2044 ESTRATÉGIAS", phase_current=idx, phase_total=len(base))
                ranked.sort(key=lambda x: (x["profit"], x["rate"]), reverse=True)
                ranked = ranked[:max_count_req]
                ranking_cache[rank_key] = ranked
                max_count = min(max_count_req, len(ranked))
                if max_count <= 0:
                    continue
                ...
                for count in range(1, max_count + 1):
                    ...
                    for pos in cand["positions"]:
                        ...
                        for con in consensuses:
                            t = _optimizer_consensus_target(vr, vb, con)
                            if contrary and t:
                                t = 2 if t == 1 else 1
                    ...
                    for con in consensuses:
                        r = _optimizer_simulate_mask(colors, masks_by_consensus[con], targets_by_consensus[con], cfg, gale, mode)
                        tested += 1
                        rate = float(r.get("rate", 0.0))
                        dd = float(r.get("max_dd", 0.0))
                        profit = float(r.get("profit", float("-inf")))
                        passes = True
                        if min_rate_enabled and rate + 0.0001 < min_rate:
                            passes = False
                        ...
                        if tested % 25 == 0:
                            _optimizer_job_update(
                                job_id, phase="VARREDURA EXATA NO SERVIDOR", tested=tested, total=total_configs,
        ...
        _optimizer_job_update(job_id, status="done", phase="CONCLUÍDO", progress=1.0, tested=tested, total=total_configs, result=result_payload)
    except Exception as exc:
        _optimizer_job_update(job_id, status="error", phase="ERRO", error=str(exc), progress=1.0)
        print("Optimizer remoto erro:", repr(exc))
...
def main():
    print("V56.0 OTIMIZADOR REMOTO + BRANCO MANUAL: janela 30000 + branco não autoativa + SSL FIX: iniciando servidor...", flush=True)
    carregar_estado()
