...
def _optimizer_warmup_background():
    ...
    try:
        ...
        print(
            "V58.3 optimizer JIT pre-aquecido em %.3fs" % (time.time() - t0),
            flush=True
        )
    except Exception as exc:
        print("V58.3 warmup optimizer falhou:", repr(exc), flush=True)


def main():
    print("V58.3 ACERTO MÁXIMO BUSCA CORRETA + BASE AUDITADA + COLETA REDUNDANTE: iniciando servidor...", flush=True)
    carregar_estado()
